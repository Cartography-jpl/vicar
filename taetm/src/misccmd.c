...
/* name    type      k  m maxc    size        dc  val      dvp*/
    ...
    static TEXT	*interruptm[] = {"INTERRUPT", "NOSELFTUTOR"};	
 ...
 */
CODE	body_do(struct CONTXT*,struct CONTXT*),	cmd_do(struct CONTXT*,struct CONTXT*), 	cont_do(struct CONTXT*,struct CONTXT*), 	defcmd_do(struct CONTXT*,struct CONTXT*),
    		delcmd_do(struct CONTXT*,struct CONTXT*), 	delgbl_do(struct CONTXT*,struct CONTXT*), 	delloc_do(struct CONTXT*,struct CONTXT*),	delvar_do(struct CONTXT*,struct CONTXT*),
		exit_do(struct CONTXT*,struct CONTXT*),      getpar_do(struct CONTXT*,struct CONTXT*), 	global_do(struct CONTXT*,struct CONTXT*), 	logoff_do(struct CONTXT*,struct CONTXT*),
    		proced_do(struct CONTXT*,struct CONTXT*), 	process_do(struct CONTXT*,struct CONTXT*), 	return_do(struct CONTXT*,struct CONTXT*), 	stop_do(struct CONTXT*,struct CONTXT*),
	    	subcmd_do(struct CONTXT*,struct CONTXT*),	parmset_do(struct CONTXT*,struct CONTXT*),  switch_do(struct CONTXT*,struct CONTXT*), 	
    		abort_do(struct CONTXT*,struct CONTXT*),     nop_do(struct CONTXT*,struct CONTXT*);
...
static FUNCTION VOID setOptions 
(
    struct CONTXT	*cpctx,		/* in:  containing proc context	*/
...
if (v)
    {
    (*cpctx).interrupt = 
      search_vector ((TEXT**)(*v).v_cvp, (*v).v_count, "INTERRUPT");
    (*cpctx).selftutor = 
      search_vector ((TEXT**)(*v).v_cvp, (*v).v_count, "SELFTUTOR") &&
		     !search_vector ((TEXT**)(*tutoropt_gbl).v_cvp, (*tutoropt_gbl).v_count, 
		"NO_SELFTUTOR");
    (*cpctx).compress = search_vector 
      ((TEXT**)(*v).v_cvp, (*v).v_count, "COMPRESSTUTOR") 
			||  
			search_vector 
      ((TEXT**)(*tutoropt_gbl).v_cvp, (*tutoropt_gbl).v_count, 
							            "COMPRESS");
    }
}
...
dc_abberr:
    tmmsg(PROCFAIL, "Abbreviations must contain at least one character",
	  "TAE-BADABB",0,0,0,0,0);
    return(DO_CHECK);

dc_cnflct:
    tmmsg(PROCFAIL, "New command conflicts with existing command '%s'.",
	  "TAE-CMDCNFLCT", (uintptr_t)SVAL(*dv, mi),0,0,0,0);
    return(DO_CHECK);

dc_noroom:
    tmmsg(PROCFAIL,
	  "Maximum number of DEFCMDs exceeded.", "TAE-MAXDEFC",0,0,0,0,0);
    return(DO_CHECK);
    }

/*
...
pdf_error:
    if (delctx.proctype != Y_UNKNOWN  &&  delctx.proctype != Y_GLOBAL)
      tmmsg(PROCFAIL, nongbl_msg, nongbl_key,0,0,0,0,0);
    f_close(&prcfil, F_KEEP);
    goto re_open;
...
undef_err:
    tmmsg(PROCFAIL,	"Reference to undefined global '%s'.",
	  "TAE-UNDEFGBL", (uintptr_t)(*v).v_name,0,0,0,0);
    goto close_ctx;
...
(
	...
	if (s_index (name, '.')  >  0)		/* is name qualified?	   */
		...
		"Delete not allowed for (qualified) variable '%s'.", 
		   "TAE-QUALDEL", (uintptr_t)name,0,0,0,0);
	    continue;
	    }
...
refcnt_err:
    tmmsg(PROCFAIL,	"Attempt to delete currently referenced global '%s'.",
	  "TAE-DELREF", (uintptr_t)(*gv).v_name,0,0,0,0);
    return (FAIL);
    }

/*
...
(
	...
	tmmsg(PROCFAIL,    "Attempt to delete implicit local variable '%s'.",
	      "TAE-DELVAR",(uintptr_t)(*vptr).v_name,0,0,0,0);
	return (FAIL);
	}
    return (SUCCESS);
...
used_err:
    tmmsg(PROCFAIL,     "Attempt to delete currently referenced local '%s'.",
	  "TAE-DELREF", (uintptr_t)(*vptr).v_name,0,0,0,0);
    return (FAIL);
    }

/*
...
(
	...
	if ((*srch).type == SRCH_GOTO)
		...
		tmmsg (PROCFAIL, msg, "TAE-BADONFAIL", (uintptr_t)(*srch).label,0,0,0,0);
		}
	    else
		tmmsg (PROCFAIL, "Reference to undefined label '%s'.",
		       "TAE-UNDLABEL", (uintptr_t)(*srch).label,0,0,0,0);
	    return (DO_RETURN);
	    }
...
(
    ...
    if (!(*cpctx).subblk)
	{
	  tmmsg(PROCFAIL, "END-SUB with no matching SUBCMD.", "TAE-NOSUB",0,0,0,0,0);
	return(DO_RETURN);
	}
...
(
	...
	{
	  tmmsg (PROCFAIL, "'EXIT' command has been disabled.", "TAE-DISHOST",0,0,0,0,0);
	return (DO_CHECK);
	}	
    ...
    disable_recvar (0,0);		/* DISABLE-RECVAR 		*/
...
(
    ...
    if (parent_runtype == BATCH || parent_runtype == ASYNC)
    	{
    	tmmsg (PROCFAIL,
	       "Async jobs may request parms only if invoked interactively.", "TAE-ASYPARM",0,0,0,0,0);
    	return (DO_CHECK);
    	}
#endif
    if (run_type == BATCH)
...
/*	Construct the name of the PROC with subcommand */
    ...
    pt = procspec;
    code = cre_locstr (&(*parctx).parmst, "_PROC", 1, FSPECSIZ+1, (GENPTR) &pt);
    if (code != SUCCESS)
    	return (FAIL);
    ...
    if (code != SUCCESS)
    	{
    	tmmsg(PROCFAIL,
	      "Error opening async communication file.  Host code = %s.","TAE-COMOPEN", (uintptr_t)sf.errmsg,0,0,0,0);
    	return(FAIL);
    	}
    code = save_pfile (&sf, M_SUBPDF, NULL, 0, &(*parctx).parmst, 0, 0);	/* write parmst to file */
    if (code != SUCCESS)
    	{
    	tmmsg(PROCFAIL,
	      "Error writing to async communication file. Host code = %s.","TAE-COMWRT", (uintptr_t)sf.errmsg,0,0,0,0);
    	f_close(&sf, F_DELETE);
    	return(FAIL);
    	}
    f_close (&sf, F_KEEP);
...
(
    ...
    {
    struct VARIABLE	*v;
    struct IPBLOCK	*ipbpt = 0, *pt, *lastpt;
    struct SEARCH	*srch;
    CODE		code;
	...
	(*cpctx).srchblk = (GENPTR) srch;	/* Put search block pointer in ctx */
		...
		for (pt = (struct IPBLOCK *)(*cpctx).int_procs; pt != NULL; pt = (*pt).flink)
			...
			tmmsg (PROCFAIL, "NAME '%s' for internal proc already in use.",
			       "TAE-DUBINTNAME", (uintptr_t)(*ipbpt).procname,0,0,0,0);
			return (DO_RETURN);
			}
		    }
...
(
    ...
    for (i=0; i<count; i++)
    	...
    	if (vold == NULL)
		  ...
		  "TAE-UNDEFVAR", (uintptr_t)names[i],0,0,0,0);
    	    return(FAIL);
    	    }
...
(
    ...
    if ((*cpctx).proctype == Y_UNKNOWN)		/* beginning of PDF */
        ...
        addstr2(&((*cpctx).locst), "_ACTION", 1, 1, (TEXT**)(*v).v_cvp, V_LOCAL);
...
(
    ...
    if (s == NULL)
	{
	  tmmsg(PROCFAIL, "TAE Monitor internal memory overflow.", "TAE-MEMOVR",0,0,0,0,0);
	return(DO_RETURN);
	}
	...
	{
	  tmmsg(PROCFAIL, "Nesting of subcommands not allowed.", "TAE-SUBNST",0,0,0,0,0);
	return(DO_RETURN);
	}
	...
	if ((*cpctx).subfnd)			/* if correct subc already found*/
	  tmpsub[(int)(*cpctx).subsiz] = EOS;	/* shorten to orig typed length*/
	    					/* for ambiguity check		*/
	if (s_equal((*npctx).subcmd, "DEFAULT")  &&  NULLSTR(tmpsub))
		...
		tmmsg(SUCCESS,
		    "Subcommand '%s' specified twice or '%s' ambiguous.",
		      "TAE-SUBTWC", (uintptr_t)SVAL(*v, 0), (uintptr_t)tmpsub,0,0,0);
...
(
    ...
    if (code != SUCCESS)
        {
	  tmmsg(PROCFAIL, "Invalid hexadecimal value.", "TAE-INVHEX",0,0,0,0,0);
	return (DO_CHECK);
	}
    IVAL(*switch_gbl,0) = (int) value;
    return (DO_SUCCESS);
    }

/*
 ...
 */
    ...
    *defv = NULL;
    for (ptr_cmd = cmd;   *incmd != EOS; )
      *ptr_cmd++ = s_table [(int)*incmd++];	/* make upper case	*/
	...
	*defv = dv;				/* set for caller	*/
	for (*index = 0; *index < (*dv).v_count; (*index)++)	
	    ...
	    if (cmd[0] != s_table [(int)*defstr])	/* for speed		*/
	        continue;
