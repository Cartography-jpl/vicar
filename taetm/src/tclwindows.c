 ...
 * 10-aug-94	PR2830: changed addint to take maxc, count, valvec as parameters.
 *		Note: "_SELECT" qualifiers can have more than one value...krw
 ***************************************************************************/
...
FUNCTION static CODE setOutputValue 
(
...

CODE	sleep_do(struct CONTXT*, struct CONTXT*);
CODE	window_init_do (struct CONTXT*, struct CONTXT*), window_create_do(struct CONTXT*, struct CONTXT*), window_clear_do (struct CONTXT*, struct CONTXT*),
	window_clear_do (struct CONTXT*, struct CONTXT*), window_delete_do (struct CONTXT*, struct CONTXT*), window_modify_do (struct CONTXT*, struct CONTXT*),
	window_raise_do (struct CONTXT*, struct CONTXT*), window_lower_do (struct CONTXT*, struct CONTXT*), window_draw_do(struct CONTXT*, struct CONTXT*), 
	window_list_do(struct CONTXT*, struct CONTXT*), window_text_do(struct CONTXT*, struct CONTXT*), window_colormap_do(struct CONTXT*, struct CONTXT*),
	window_info_do(struct CONTXT*, struct CONTXT*); 

CODE	Wpt_Panel_do (struct CONTXT*, struct CONTXT*), Wpt_Event_do (struct CONTXT*, struct CONTXT*), Wpt_CloseDisplay_do (struct CONTXT*, struct CONTXT*);
CODE	Wpt_Combined_do (struct CONTXT*, struct CONTXT*), Wpt_Message_do (struct CONTXT*, struct CONTXT*);
CODE    Wpt_Reject_do (struct CONTXT*, struct CONTXT*), Wpt_ParmUpdate_do (struct CONTXT*, struct CONTXT*), Wpt_SetPanelState_do (struct CONTXT*, struct CONTXT*);
CODE	Wpt_ViewUpdate_do (struct CONTXT*, struct CONTXT*), Wpt_Rehearse_do (struct CONTXT*, struct CONTXT*);
CODE 	Wpt_Script_do (struct CONTXT*, struct CONTXT*), Wpt_SetJob_do (struct CONTXT*, struct CONTXT*);
 ...
 *
 */
    FUNCTION CODE Wpt_CloseDisplay_do(cpctx, npctx)
...
FUNCTION CODE window_colormap_do(struct CONTXT* UNUSED(X1), struct CONTXT* UNUSED(X2))
     {
#ifdef XWINDOWS
     Status status;
     COUNT  pixel;
...
/* FUNCTION static CODE translateFunc  */
...
(
	...
	COUNT i;
	    ...
	    if ((*qualv).v_type == V_INTEGER) 
	        addint (&(*targetv).v_qualst, (*qualv).v_name,
			(*qualv).v_maxc, (*qualv).v_count,
			(FUNINT*)(*qualv).v_cvp,  (*targetv).v_class);
	    else if ((*qualv).v_type == V_STRING)
    		addstr2(&(*targetv).v_qualst, (*qualv).v_name, 
			(*qualv).v_maxc, (*qualv).v_count, 
		       (TEXT**)(*qualv).v_cvp,   (*targetv).v_class);
		...
		v = allvar(&(*targetv).v_qualst);
    		...
    		(*v).v_type    = V_REAL;
		(*v).v_minc = 1;
		(*v).v_maxc = (*qualv).v_maxc;
    		(*v).v_count = (*qualv).v_count;
    		(*v).v_dcount  = 0;
    		(*v).v_default = FALSE;
    		(*v).v_size    = 0;
		...
		for(j=0; j<(*v).v_count; ++j)
    		    civ[j] = (TAEFLOAT) (*qualv).v_cvp[j];
		}
	    else
		continue;
	    targetSel = lookex (&(*targetv).v_qualst, (*qualv).v_name); /* should be there now */
	    }

	if ((*qualv).v_type == V_INTEGER && s_equal((*qualv).v_name, "_SELECT"))
	    {
	    FAST int j;
	    for (j=0; j<(*qualv).v_count; ++j)
		IVAL(*qualv, j)++;  /* Special case for "_SELECT", Convert to TCL type index */
	    }

	/* Now update the qualifier with the new value(s). */
	...
	s_copy ((*qualv).v_name, &qualTargetName[i]);
		...
		min((*targetSel).v_maxc, (*qualv).v_count), (*qualv).v_type);
	}
    return (0);
    }

...
/*     FUNCTION static VOID notInitialized (void) */

/*     { */
/*     tmmsg (PROCFAIL, "Window system not initialized. Type WPT-INIT.", */
/*            "TAE-NOXINIT",0,0,0,0,0); */
/*     return ; */
/*     } */

...
/* FUNCTION static VOID extractParent  */
/* ( */
 ...
 */
    ...
    vptr[0] = (*npctx).subcmd;
    addstr2(&((*npctx).locst), "_SUBCMD", 1, 1, vptr, V_LOCAL);
    vptr[0] = (*cpctx).pdf.name;
    addstr2(&((*npctx).locst), "_PROC",   1, 1, vptr,   V_LOCAL);
    intVal[0] = (*cpctx).pdf_line+1;
    addint(&((*npctx).locst), "_LINE", 1, 1, intVal, V_LOCAL);
