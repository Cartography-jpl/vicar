...
FUNCTION  static  CODE  disp_summary
(
...
FUNCTION static CODE get_hinput
(
...
FUNCTION CODE lookahead
(
    struct DIRBLK	*dirblk		/* in: directive control block	*/
 );
FUNCTION  static  CODE  gthlpcmd
(
...
FUNCTION  static VOID noscr_help
(
    struct DIRBLK	*dirblk,	/* in: directive control block pointing	*/
					/*     to start of help text		*/
    TEXT		hdr[],		/* in: header				*/
...
FUNCTION static VOID parms_hdr
(
...
FUNCTION static VOID parms_getval
(
...
FUNCTION static VOID parms_dspval
(
...
FUNCTION static CODE disp_page
(
    ...
    numlines = 0;
    do {code = d_text(dirblk, inlinex);} while (code==D_PAGE);	/* skip leading .pages */
    *lastpage = FALSE;			/* assume not last page		*/
    ...
    while (numlines < max_lines)
	    ...
	    code = d_text(dirblk, inlinex);	/* get next line    	     */
	if (code == D_EOT)
    	    {
    	...
    	if (code == D_PAGE)
    	    break;
    	put_line(numlines+H_HDR_SIZE+titlesize, 1, inlinex, TRUE);	/* write the line    */
...
FUNCTION  static CODE  disp_help
(
...
(
	...
	if (code != SUCCESS || pagenum <= 0)
	    {
	      help_err(pagen_msg, "TAE-PAGENUM",0,0,0,0,0);
    	    code = FAIL;
	    }
	else
...
(
	...
	if (i < NHCMD)					/* if match found    */
		...
		return (i+1000);			/* return cmd type	*/
	      ...
	      help_err(errmsg, errkey,0,0,0,0,0);		/* report error		*/
	    }
...

    ...
    do
	{
	  code = f_bread(fctx, (GENPTR) &inxrec, sizeof(inxrec), &recsize);  /* read index file record */
...
FUNCTION static CODE pos_subcmd
(
    struct DIRBLK	*dirblk,	/* In: directive control block		*/
    TEXT		procname[],	/* In: proc or command name		*/
