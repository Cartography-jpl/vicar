...
CODE disable_do(struct CONTXT* ,struct CONTXT*), enable_do(struct CONTXT* ,struct CONTXT*), putmsg_do(struct CONTXT* ,struct CONTXT*),
	...
	CODE enable_pdf_force_do(struct CONTXT* ,struct CONTXT*), disable_pdf_force_do(struct CONTXT* ,struct CONTXT*);
...
{4, "DISABLE",  "PDF_LOWER", Y_GENERAL,       0,      NULL, disable_pdf_force_do},
...
{3, "ENABLE", "PDF_LOWER", Y_GENERAL,        0,      NULL,   enable_pdf_force_do},
...

/*	enable/disable -PDF_LOWER.
 *
 *	PDF_LOWER is like FORCE_LOWER, but it only make the end part
 *	of the file spec lower case. This allows mixed case directories, with
 *	lower case PDF names.
 *
 */

FUNCTION CODE enable_pdf_force_do (struct CONTXT* UNUSED(X1) ,struct CONTXT* UNUSED(X2))
	{
	f_pdf_lower(TRUE);
	return (DO_CHECK);
	}
	...
	FUNCTION CODE disable_pdf_force_do(struct CONTXT* UNUSED(X1) ,struct CONTXT* UNUSED(X2))
	{
	f_pdf_lower(FALSE);
	return (DO_CHECK);
	}
