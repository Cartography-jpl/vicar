...
/*
 * Do a cmd_parse for directory search, except in the special case of
 * a string that starts with a -, in which case we pass the string
 * though unchanged minus this leading -. This allows support for
 * things like directories with spaces, or with the character
 * '-'. This is a bit klunky, but
 * we are fitting this functionality into an existing code and I don't 
 * want to break any existing functionality.
 */
 

FUNCTION CODE dir_parse 
(
 ...
 )
{
  CODE res = SUCCESS;
  if(cmdstr[0] == '-') {
    strncpy(cmd, cmdstr + 1, FSPECSIZ+1);
    subcmd[0] = EOS;
    subcmd[1] = 0;
  } else {
    res = cmd_parse(sb, cmdstr, label, cmd, subcmd);
  }
  return res;
}
...
(
	...
	if ((*usrl_gbl).v_count > 0)
	    {
	    dir_parse(&sb, SVAL(*usrl_gbl,0), NULL,
	...
	if (v != NULL)
	    ...
	    dir_parse(&sb, SVAL(*sysl_gbl,0), NULL,
