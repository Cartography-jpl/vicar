 ...
 *      18-jul-95       Solaris would not honor a CNTRL-C.  Had a lot to
 *                      do with the way SYSV conditionals were constructed.
 *                      I have reworked them for sun systems.  Also a few
 *                      bugs were fixed in the process function that would
 *                      render CNTRL-C problematic.  Kris Becker, USGS
 *      26-jan-96       Made numerous changes so applications can be
 *                      debugged in the Solaris environment.  Kris Becker, USGS
 */
...
FUNCTION  static VOID  c_suspend
(
    pid_t		child_pid /* process id of task to suspend */

 );
FUNCTION  static VOID  c_resume
(
    pid_t		child_pid /* process id of suspended task */
...
#include	<sys/socket.h>
    ...
    volatile COUNT	already_read = 0;
#ifdef __alpha
    volatile COUNT	already_wrote;
#else
    volatile COUNT	already_wrote = 0;
#endif
    GENPTR		p;

    sigset_t block_sigread;
...
/* first make sure we get a good library name by finding the parent pdf */
    ...
    f_spec(&fsblock, exespec);			/* build file spec to
						   run  */
    if(Force_lower)
      s_lower(exespec);				/* convert to lower
						   case   */
    if(Pdf_lower)
      pdf_lower(exespec);	/* Convert PDF part of file name to
				   lower case */
...
/* build the parblk        */
    if (package(pctx, (struct PARBLK*)&parblk, sizeof parblk.pool, packageFlag) != SUCCESS)		goto rp_overr;	
    ...
    sigemptyset( &block_sigread );
    sigaddset( &block_sigread, SIGREAD );
    sigprocmask( SIG_BLOCK, &block_sigread, NULL );    /* ignore the children for now */
...
/*******   now process the message    ************/
    	...
    	if (chk_parblk ((struct PARBLK*)&parblk) != SUCCESS)
	    goto rp_badparblk;
	if (parblk.msgtyp == M_FULLPDF  ||  parblk.msgtyp == M_SUBPDF)
		...
		{
#ifdef TAE_ASYNC
		  code = get_pblk_remote(pctx, (struct PARBLK*) &parblk, errmsg);
		...
		asytut_req = FALSE;		/* tutor for sync proc	*/
						/* dynamic parameter    */
		dynget(pctx, (struct PARBLK*)&parblk, sizeof ( parblk.pool), asytut_req);
		logdyn((struct PARBLK*)&parblk);		/* log dynamic parameters   */
		}
	    /*set_signal (SIGREAD, pipe_sig);	 reenable signal from task */
	    ...
	    {
	      dyncommand (pctx, (struct PARBLK*)&parblk, sizeof parblk.pool);
	    /*set_signal (SIGREAD, pipe_sig);	 reenable signal from task */
	    count = write(write_chan, &ackn_msg, ACKN_SIZE);	/* send ackn */
						/* now send parameter block  */
            already_wrote = 0;
	    ...
	    {
	      logmsg(pctx, (struct PARBLK*)&parblk);		/* log the message           */
	    /*set_signal (SIGREAD, pipe_sig);	 reenable signal from task */
	    count = write(write_chan, &ackn_msg, ACKN_SIZE);	/* send ackn */
	    ...
	    {
	      code = outputs(pctx, (struct PARBLK*)&parblk);	/* apply output values	     */
	    ...
	    {
	      dynamic_ins (pctx, (struct PARBLK*)&parblk);	/* installation exit	     */
	    /*set_signal (SIGREAD, pipe_sig);    reenable signal from task */
	    count = write(write_chan, &ackn_msg, ACKN_SIZE);	/* send ackn */
	    }
...
WAITLOOP:
     ...
     set_signal (SIGREAD, pipe_sig);
     sigprocmask( SIG_UNBLOCK, &block_sigread, NULL );    /* now ready to listen */
...
(
    ...
    /* Under Linux, signal delivery is blocked while the signal is	*/
    /* being handled.  Unfortunately, longjmp() does not unblock it...	*/
    /* so only one signal gets delivered.  This is prevented by turning	*/
    /* on SA_NOMASK, so the signals are never blocked.  We could	*/
    /* alternatively call sigsetjmp() and siglongjmp(), but since the	*/
    /* signal environment is changed after setjmp() is called, that	*/
    ...
    act.sa_flags = 0;
    /*act.sa_flags = SA_NOMASK;*/ /* renamed SA_NODEFER ? */
...
/* Ernest Cisneros, Flagstaff Field Center, USGS 95.03.17            */
/* Not very generous size, broke with our installation`s environment */
/* so we increased value to 200 in an effort to accomodate our users */
/* #define MAXENV 100 */
/* Change: July 2, 1996		By: Ernest Bowman-Cisneros */
/* Now we've had to change it to 1000, for the environment hungry */
/* VICAR software */
/*#define	MAXENV 200 */
#define	MAXENV 1000
...
/*  Exec requested task or debugger with requested task.		*/
    ...
    if (*child_pid == -1)
        ...
        {
	if (debug)
            execle(dbg_name, dbg_name, task_name, NULL, envlist);
	else
	    execle(task_name, task_name, NULL, envlist);
	_exit(1);
        }
...
/*  Accept connection to socket.					*/
    ...
    FD_ZERO(&readfds);
    FD_SET(sd, &readfds);
    nfds = select(sd+1, &readfds, 0, 0, 0);
...
FUNCTION static VOID chld_sig(int UNUSED(X))
{
}

/*
...
(
    ...
    {
/* #ifndef SYSV */
#if !defined(SYSV) || defined(sun) || defined(__linux) || defined(linux)
...
/*******   now process the message    ************/
	...
	while ((code = wait(&status)) != child_pid  && code != -1)
		...
		code = interrupt(pctx);	/* do interrupt processing 	 */
		    ...
		    {
		    kill(child_pid, SIGKILL);	/* abort the task	 */
                    ret_code = DO_ABORT;        /* Added by KJB - 95-Jul-18 */
		    goto close_ret;
		    }
		else
		    c_resume(child_pid);	/* resume suspended task */
 ...
 */
    ...
    VOID int_sig()			/* here for operator attn	*/
    {
    op_int = TRUE;
 ...
 */
    ...
    VOID pipe_sig()			/* here when child wants to send */
