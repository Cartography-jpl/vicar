 ...
 *	25-may-93	$TAEBIN now is what $TAEBIN/$TAEPLAT used to be...kbs
 *
 *****************************************************************************
 */
	...
	if(Force_lower)
	  s_lower(que_name);
	    ...
	    tmmsg(PROCFAIL, "Invalid batch queue name '%s' specified.",
		  "TAE-INVBQNAM", (uintptr_t) SVAL(*v1, 1), 0, 0, 0, 0);
	    return (FAIL);
	    }
	}
...
(
    ...
    sprintf(bch_block.name, "job%ld_%d", pid, job_number);  /* file name	     */
...
openerr:
    tmmsg (PROCFAIL, "Unable to create batch job file (%s). %s",
    	   "TAE-BCHOPN", (uintptr_t) job_name, (uintptr_t) bch_file.errmsg, 
	   0, 0, 0);
    return (DO_CHECK);
    }

