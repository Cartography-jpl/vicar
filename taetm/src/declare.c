...
FUNCTION static CODE set_p_quals 
(
...
/* name    type      k  m maxc      size     dc val      dvp	*/
	  ...
	  {"INITIAL", V_STRING, 0, 0, MAXVAL, MAXSTRSIZ,0, NULL,   NULL},
	  {"VALID",   V_STRING, 0, 1, MAXVAL, VALIDSIZ,  1, NULL,   (GENPTR)null_string},
	  {"ACCESS",  V_STRING, 1, 1, 1,      MAXSTRSIZ,1, (GENPTR)&access, (GENPTR)nocheck},
...
/* name    type      k  m maxc      size     dc val      dvp	*/
	  ...
	  {"TYPE",    V_STRING, 0, 1, 2,      NAMESIZ,  1, NULL,   (GENPTR)defstring},
	  {"COUNT",   V_STRING, 0, 1, 2,      NAMESIZ,  1, NULL,   (GENPTR)one},
	  {"DEFAULT", V_STRING, 0, 0, MAXVAL, MAXSTRSIZ,0, NULL,   NULL},
	  {"VALID",   V_STRING, 0, 1, MAXVAL, VALIDSIZ,  1, NULL,   (GENPTR)null_string},
	  {"ACCESS",  V_STRING, 1, 1, 1,      MAXSTRSIZ,1, (GENPTR)&access, (GENPTR)nocheck},
	  {"QUALS",   V_STRING, 0, 0, 1,      MAXSTRSIZ,0, NULL,   NULL}
	};
...
/* 	  {"TYPE",    V_STRING, 0, 1, 2,      NAMESIZ,  1, NULL,   (GENPTR)defstring}, */
/* 	  {"COUNT",   V_STRING, 0, 1, 2,      NAMESIZ,  1, NULL,   (GENPTR)one}, */
/* 	  {"TITLE",   V_STRING, 0, 0, MAXVAL, MAXSTRSIZ,0, NULL,   NULL}, */
/* 	  {"VALID",   V_STRING, 0, 1, MAXVAL, VALIDSIZ,  1, NULL,   (GENPTR)null_string}, */
/* 	  {"ACCESS",  V_STRING, 1, 1, 1,      MAXSTRSIZ,1, (GENPTR)&access, (GENPTR)nocheck}, */
...
/* 	}; */
    ...
    CODE	view_do();
#endif
CODE parm_do(struct CONTXT*,struct CONTXT*);
CODE local_do(struct CONTXT*,struct CONTXT*);
CODE refgbl_do(struct CONTXT*,struct CONTXT*); 
CODE defgbl_do(struct CONTXT*,struct CONTXT*);
CODE refview_do(struct CONTXT*,struct CONTXT*);
 ...
 */
FUNCTION static struct VARIABLE *addvar 
(
    ...
    struct VARIABLE	*tv = 0; /* target (new) variable	*/
...

	...
	if ((*tv).v_class == V_PARM  &&  (*tv).v_keyword  &&  (*vv).v_default)
	    goto valid_required;
        if (!(*vv).v_default)
	  if (bld_valid(destst, tv, (TEXT **) (*vv).v_cvp, (*vv).v_count) != SUCCESS)
		goto dp_badvalid;
...
dp_badref:
    tmmsg(PROCFAIL, "Undefined DEFAULT variable reference '%s'.",
          "TAE-BADREF", (uintptr_t) SVAL(*pv,0), 0, 0, 0, 0);
...
bad_ref:
    tmmsg(PROCFAIL, "Inconsistent re-definition of existing global '%s'.",
          "TAE-REDEF", (uintptr_t) (*gl).v_name, 0, 0, 0, 0);
    return (FAIL);

bad_qref:
    tmmsg(PROCFAIL, "Re-definition of existing global omits qualifier '%s'.",
          "TAE-REDEF", (uintptr_t) (*gvq).v_name, 0, 0, 0, 0);
	...
	if ((*procctx).special == COMPILING)
		...
		if ((*defd_var).v_class == V_GLOBAL  &&  (*defd_var).v_implicit)
		    ...
		    tmmsg(PROCFAIL, "Variable '%s' previously defined or referenced.",
			  "TAE-REFEX", (uintptr_t) SVAL(*nv,i), 0, 0, 0, 0);
		    return(DO_RETURN);
		    }
		}
		...
		tmmsg(PROCFAIL, "Reference to undefined global variable '%s'.",
		      "TAE-UNDEFGBL", (uintptr_t) SVAL(*nv, i), 0, 0, 0, 0);
		return(DO_RETURN);
		}
...
dp_dferr:
    tmmsg(PROCFAIL, "Invalid DEFAULT or INITIAL specification for '%s'.",
	  "TAE-DFTERR", (uintptr_t) (*tv).v_name, 0, 0, 0, 0);
    return(FAIL);
    }

/*
