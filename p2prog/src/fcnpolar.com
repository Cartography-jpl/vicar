...
$! VPACK Version 1.8, Tuesday, October 07, 1997, 14:23:41
$!
...
c      open(11,name=polartbl,type='old',err=999)
      open(11,name=polartbl,status='old',err=999)
      read(11,1)
      read(11,1)
1     format(a)
      do 10 I=1,500
...
50    close(11,err=999)
	 ...
	 d(j)=abs(answer(1)+answer(2)*(cosd(array2(j)-answer(3))**2)
     +   -array(j))
         sres=sres+abs(d(j))
...
c (1)=radius (2)=x_center (3)=y_center
      ...
      m=0
      do j=1,n
        dr(j)=abs(x(1)+x(2)*(cosd(array2(j)-x(3))**2)-array(j))
        sum=sum+dr(j)
