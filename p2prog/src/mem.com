...
C		OPEN THE CONVOLVED (BLURRED) IMAGE
	...
	CALL XVOPEN (UNIT(DATA), STATUS, 'IO_ACT', 'SA', 'OPEN_ACT', 'SA',
     +		'U_FORMAT', 'REAL',' ')
...
C		COPY CURRENT IMAGE SCRATCH INTO OUTPUT MEM IMAGE
	...
	DO LINE = 1, NL
	    CALL XVREAD(UNIT(CURRENT), IMAGE(1,CURRENT), STATUS, 'LINE',LINE,
     +                   ' ')
     ...
     +			   MIN( IMAGE(SAMP,CURRENT) + 0.5, MAXFORMAT)
		ENDDO
	    ENDIF
	    CALL XVWRIT (UNIT(OUT), IMAGE(1,CURRENT), STATUS, 'LINE',LINE,' ')
	ENDDO
	CALL XVCLOSE (UNIT(OUT), STATUS,' ')
...
C		OPEN AND COPY THE DEFAULT IMAGE IF THERE IS ONE
	...
	IF (DEFFLAG) THEN
     ...
     +						 'LINE',LINE,' ')
		...
		ENDDO
		CALL XVWRIT (UNIT(DEF), IMAGE(1,DEF), STATUS, 'LINE',LINE,' ')
	    ENDDO
...
C		    IN ORDER TO MAKE THE STARTING RESIDUALS
	...
	DO LINE = 1, NL
	    CALL XVREAD(UNIT(DATA), IMAGE(1,DATA), STATUS, 
     +					'LINE',LINE+SL-1, 'SAMP',SS,' ')
	    CALL XVREAD(UNIT(RESID), IMAGE(1,RESID), STATUS, 'LINE',LINE,' ')
	    DO SAMP = 1, NS
		IMAGE(SAMP,RESID) = IMAGE(SAMP,RESID) - IMAGE(SAMP,DATA)
		ERROR = ERROR + IMAGE(SAMP,RESID)**2
	    ENDDO
	    CALL XVWRIT (UNIT(RESID), IMAGE(1,RESID), STATUS, 'LINE',LINE,' ')
	ENDDO
	...
	IF (RESTART) THEN
     ...
     +				'LINE',LINE,' ')
	        CALL XVREAD(UNIT(DEF), IMAGE(1,DEF), STATUS, 'LINE',LINE,' ')
	        CALL XVREAD(UNIT(GRDE),IMAGE(1,GRDE),STATUS,'LINE',LINE,' ')
	        DO SAMP = 1, NS
	...
	DO LINE = 1, NL
	    CALL XVREAD(UNIT(CURRENT), IMAGE(1,CURRENT), STATUS, 'LINE',LINE,
     +                    ' ')
	    CALL XVREAD(UNIT(STEP), IMAGE(1,STEP), STATUS, 'LINE',LINE,' ')
	    DO SAMP = 1, NS
		IMAGE(SAMP,CURRENT) = IMAGE(SAMP,CURRENT)
     +				+ STEPLEN * IMAGE(SAMP,STEP)
		IMAGEMIN = MIN (IMAGEMIN, IMAGE(SAMP,CURRENT) )
	    ENDDO
	    CALL XVWRIT (UNIT(CURRENT), IMAGE(1,CURRENT), STATUS, 'LINE',LINE,
     +                   ' ')
	ENDDO
	...
	DO LINE = 1, NL
	    CALL XVREAD(UNIT(RESID), IMAGE(1,RESID), STATUS, 'LINE',LINE,' ')
	    CALL XVREAD(UNIT(CNVSTP),IMAGE(1,CNVSTP),STATUS,'LINE',LINE,' ')
	    DO SAMP = 1, NS
...
C	TYPE *,'     Alpha              : ', ALPHA
	...
	DO LINE = 1, NL
     ...
     +			  (0.1*IMAGEMIN-IMAGE(SAMP,CURRENT))/MAXSTEPLEN )
	    ...
	    ENDDO
	    CALL XVWRIT (UNIT(STEP), IMAGE(1,STEP), STATUS, 'LINE',LINE,' ')
	ENDDO
...
C	    C = GRADDOTGRAD(J,J) - MAXCRIT *COMPOFGRADJ
	...
	VOLUME = 0.0
	DO LINE = 1, NLPSF
	    CALL XVREAD(UNIT(PSF), IMAGE(1,PSF), STATUS, 'LINE',LINE,' ')
	    DO SAMP = 1, NSPSF
	...
	DO LINE = 1, NLPSF
		...
		IMAGE(SAMP,XFER) = 0.0
	    ENDDO
	    CALL XVWRIT (UNIT(XFER), IMAGE(1,XFER), STATUS, 'LINE',LINE,' ')
	ENDDO
	DO SAMP = 1, NSFFT
	    IMAGE(SAMP,XFER) = 0.0
	ENDDO
	...
	DO LINE = 1, NLFFT
	    CALL XVREAD(UNIT(XFER), CBUFFER, STATUS, 'LINE',LINE,' ')
	    DO SAMP = 1, NSFFT/2
...
C
	...
	SSTART = NSPSF/2 + 1
	IF (.NOT. CONJGFLAG .AND. (MOD( NSPSF, 2) .EQ. 0)) SSTART = SSTART - 1
	LSTART = 1 - NLPSF/2
	IF (CONJGFLAG .AND. (MOD( NLPSF, 2) .EQ. 0)) LSTART = LSTART + 1
	LEND = NL + LSTART - 1
...
C		    AND PAD WITH ZEROS
	DO LINE = 1, NL
	    CALL XVREAD(UNIT(INIM), IMAGE(1,INIM), STATUS, 'LINE',LINE,' ')
	    DO SAMP = 1, NS
	    ...
	    ENDDO
	    CALL XVWRIT(UNIT(TMPFFT),IMAGE(1,TMPFFT),STATUS,'LINE',LINE,' ')
	ENDDO
	DO LINE = NL+1, NLFFT
	    DO SAMP = 1, NSFFT
		IMAGE(SAMP,TMPFFT) = 0.0
...
C		    BY THE TRANSFER IMAGE
	DO LINE = 1, NLFFT
	    CALL XVREAD(UNIT(TMPFFT), FBUFFER, STATUS, 'LINE',LINE,' ')
	    CALL XVREAD(UNIT(XFER), XFERBUFFER, STATUS, 'LINE',LINE,' ')
	    DO SAMP = 1, NSFFT/2
		IF (CONJGFLAG)  XFERBUFFER(SAMP) = CONJG(XFERBUFFER(SAMP))
		FBUFFER(SAMP) = FBUFFER(SAMP) * XFERBUFFER(SAMP)
	    ENDDO
	    CALL XVWRIT(UNIT(TMPFFT), FBUFFER, STATUS, 'LINE',LINE,' ')
	    IF (CONJGFLAG)  XFERCOLBUF(LINE) = CONJG(XFERCOLBUF(LINE))
...
C		COPY INTO THE OUTPUT FILE
	DO LINE = 1, NL
	    CALL XVREAD(UNIT(TMPFFT),IMAGE(1,TMPFFT),STATUS,'LINE',LINE,' ')
	    CALL XVWRIT(UNIT(OUTIM), IMAGE(1,TMPFFT), STATUS, 'LINE',LINE,' ')
	ENDDO
...
C
	...
	IF (FFTSIGN .EQ. +1) THEN
		...
		CALL XVREAD(UNIT, BUFFER(BUFPTR), STATUS, 'LINE',LINE,' ')
	    ...
	    DO LINE = 1, NLFFT
		BUFPTR = (NSFFT/2)*(LINE-1)+1
		CALL XVWRIT(UNIT, BUFFER(BUFPTR), STATUS, 'LINE',LINE,' ')
	    ENDDO
	ELSE
	    DO LINE = 1, NLFFT
		BUFPTR = (NSFFT/2)*(LINE-1)+1
