...
c  determine whether lats are planetocentric or graphic:
	...
	call XVMESSAGE(' ** TERMINATOR option requires LIMB option also!',
	1 ' ')
	dterm = .false.
      endif
...
C    ...Add a scale bar 50 pixels wide at lower left corner.
  300 IF ((DATA(7).GT.0.).AND.(XVPTST('SCALEBAR'))) THEN
	 CALL SCALEBAR(OUNIT,DATA(7),NLO,NSO,SB,NBO,DNGRID,MAXDN,ALTERNATE,buf)
      ENDIF
...
c *********************************************************************
	SUBROUTINE LONGRID(LSTP,RMINLON,RMAXLON,NUMLON,GLON,ENDPT1,ENDPT2)
...
C       ...Write out rest of picture that has no points
        ...
        DO LINE=LINEBUF(NPTS)+1,EL
          LINEOUT = LINEOUT + 1
	  IF (IUNIT.GE.0) CALL XVREAD(IUNIT,BUF,IND,'LINE',LINE,'SAMP',SS,
	1  'NSAMPS',NSO,'BAND',BAND,' ')
	  CALL XVWRIT(OUNIT,BUF,IND,'LINE',LINEOUT,'BAND',BANDOUT,' ')
        ENDDO
