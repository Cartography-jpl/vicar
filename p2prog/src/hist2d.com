...
$! VPACK Version 1.9, Wednesday, May 28, 2003, 13:42:34
$!
...
zvopen(inunit,"OPEN_ACT","SA","IO_ACT","SA",0);
zvget(inunit,"NL",&dimensions[0],"NS",&dimensions[1],"NB",&dimensions[2],
  "FORMAT",fmt,"PIX_SIZE",&bytes,0);
...
if (bytes==1) {
  zvclose(inunit,0);
  zvopen(inunit,"OPEN_ACT","SA","IO_ACT","SA","U_FORMAT","HALF",0);
}
...
numtasks = 20;
vstat = zlhinfo(inunit,tasks,instances,&numtasks,"ULEN",10,"ERR_ACT","",0);
...
for( x=(numtasks-1); x>=0; x-- ) {
  if( vstat != 1 ) {
    vstat = zlget( inunit, "HISTORY", RAD_CONV, radiance, "HIST", tasks[x],
     "FORMAT", "REAL", "NELEMENT", -1, "NRET", &conv_values, 0);
  ...
  if( stat1 != 1 ) {
    stat1 = zlget( inunit, "HISTORY", RAD_BASE, rad_base, "HIST", tasks[x],
     "FORMAT", "REAL", "NELEMENT", -1, "NRET", &base_values, 0);
...
if( zvptst("LOG") && (maxfreq>1) ) {	/* Perform log base 10 on histogram  */
  ...
  for( bin = (out_nl-1); bin >= 0; bin-- ) {
    ...
    zvwrit(outunit,value[bin],0);		/* Write line to output file */
  }
}
else {
...
write_labelitems()
{
  ...
  zvclose(outunit,0);      /* Close histogram file      */
}
...
zvp("TOUT",testfile,&x);
zvunit(&tunit,"TOUT",1,"U_NAME",testfile,0);

	/* Open test file  */
zvopen( tunit, "OP", "WRITE",  "OPEN_ACT", "SA", "IO_ACT", "SA", "U_FORMAT",
 "FULL", "O_FORMAT", "FULL", "U_NL", nlo, "U_NS", nso, "U_NB", 1, 0);
...
for( line=1; line<=nlo; line++ ) {	/* Write histogram out to file */
  ...
  vstat = zvwrit(tunit, ln, "LINE", line, "SAMP", 1, "NSAMPS", nso, 0);
}

zvclose(tunit,0);			/* Close test output file      */
}
$ VOKAGLEVE
$ Return
$!#############################################################################
