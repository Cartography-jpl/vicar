...
$! VPACK Version 1.9, Wednesday, March 29, 2000, 14:47:37
$!
...
c   character
      ...
      data PLANET/'            '/
      data TARGET/'        '/
      DATA MSC/' /CAM FOC L/    RA   /    RB   /    RC   /   RMAG  /LA S
     +SC PT/LO SSC PT/'/
...
C         INITIALIZATION
     ...
     & // 'FILE WAS INPUT'
      call XVMESSAGE('Map3 version 6-Dec-1999',' ')
      CALL INIT_SPICE
      PI=3.141592654
      PIFAC=PI/180.
      SLAT=0.0
...
C     THESE ARRAYS ARE USEFUL IN DEFAULT SCALE AND NORTH ANGLE COMPS
      ...
      CHARACTER*3600 LABI
      CHARACTER*78 CENMSG
      DATA CENMSG/' COORDINATES OF CENTER OF INPUT PICTURE--LATITUDE =  
     +     , LONGITUDE =       '/
      ZREF(1)=1.
      XREF(1)=1.
      ZREF(2)=1.
...
c
      ...
      DATA COL4/'XXXXXXX X       SCET  YR=XX DAY=XXX GMT=XX/XX/XX       
     *                C'/
      DATA COL5/'CENTER  SL.RANGE=XXXXXXXX KM   KM/LINE=XXX.XX   KM/SAMP
     *LE=XXX.XX       C'/
      DATA COL6/'      PHASE ANG=XXX.X DEG   VIEW ANG=XX.X DEG ILLUM ANG
     *=XX.X DEG       C'/
      DATA COL7/'        LONGITUDE=XXXX.X    LATITUDE=XXXX.X            
     *                C'/
      DATA COL8/'CORNERS LONGITUDE=XXXX.X(UL)  XXXX.X(UR)   XXXX.X(LL)  
     * XXXX.X(LR)     C'/
      DATA COL9/'        LATITUDE =XXXX.X(UL)  XXXX.X(UR)   XXXX.X(LL)  
     * XXXX.X(LR)     C'/
      DATACOL10/'SUBSOLAR PIXEL S=XXXXX, L=XXXXX  SUBSPACECRAFT PIXEL S=
     *XXXXX L=XXXXX   C'/
      DATACOL11/'APPROX IMAGE SIZE  WIDTH=XXXXXXX KM  HEIGHT=XXXXXXX KM 
     *                C'/
      DATACOL12/'                                                       
     *                C'/
