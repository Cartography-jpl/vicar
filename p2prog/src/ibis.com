...
$! VPACK Version 1.9, Friday, August 22, 2003, 13:15:16
$!
...
int ibis;
{
	...
	if (indexcol)
	{
		...
		for (row=1,rowsleft=nr;rowsleft>0;rowsleft-=rowsnow)
		{
		   ...
		   for (i=0;i<rowsnow;i++)
		   	data[i] = row+i;
		   status = IBISColumnWrite(ibis,data,indexcol,row,rowsnow);
		   if (status !=1) goto failure;
		   row += rowsnow;
		}
...
static parse_user_format();
static show_header();
static show_groups();
static show_preamble();
...
list_file()
{
	...
	status = IBISFileOpen( inunit, &in, IMODE_READ, gr1dim,0,NULL,NULL);
	...
	sprintf(headerfmt, "C:%%-%dd", csize-2);
	sprintf(formatfmt, "%%-%ds", csize);
	...
	for (col=0; col<nc; col+=colperrow)
	{
		...
		for (colnum = 0; colnum<colperrow; colnum++)
		{
			...
			IBISColumnGet( in, ICOLUMN_U_SIZE, colsize+colnum, colm, 0 );
		...
		for (rows_left=nr; rows_left>0; rows_left-=rinc)
		{
			...
			{
				colm = ccnt ?   cols[col+colnum] : col+scol+colnum ;
				IBISColumnRead( in, buffer[colnum], colm, srow1, rinc );
...
static parse_user_format(userfmt,cfmt,csize)
char *userfmt;
...
static show_header(ibis,nr,nc,version)
int ibis;
int nr;
int nc;
char *version;
...
static show_groups(ibis,fmtstr,filegrp,type)
int ibis;
char *fmtstr;
char *filegrp;
char *type;
...
int copy_file()
{
	...
	for (col=0; col<nc; col++)
	{
		...
		for (row = 0; row<nr; row += row_inc)
		{
			...
			status = IBISColumnRead( in, buffer, incol, row+sr, nrow );
			if (status !=1 ) IBISSignal(in,status,1);
			status = IBISColumnWrite( out, buffer, outcol, row+1, nrow );
			if (status !=1 ) IBISSignal(out,status,1);
...
{
	int inunit,outunit,status;
	int in, out,def; /* was long; wlb 2011-06-15 */
	...
	for (i=1;i<=numout;i++)
	{
		...
		for (col=0; col<nc_file; col++)
		{
			...
			for (row = 0; row<nr_file; row += row_inc)
			{
				...
				status = IBISColumnRead( in, buffer, incol,
					 row+1, nrow );
				if (status !=1 ) IBISSignal(in,status,1);
				status = IBISColumnWrite( out, buffer, outcol,
					 row+srow[i-1], nrow );
				if (status !=1 ) IBISSignal(out,status,1);
...
modify_group()
{
	...
	status = IBISFileOpen(outunit, &ibis, IMODE_UPDATE, gr1dim, 0, NULL, NULL);
	if (status!=1) IBISSignalU( outunit, status, 1);
