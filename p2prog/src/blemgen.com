...
$! VPACK Version 1.8, Monday, July 28, 1997, 09:58:52
$!
...
/*  ==================================================================  */
    ...
    for (i = 1; i <= n_inputs; i++)
        {
	stat = zvunit(&in_unit[i-1],"INP",i,0);
	stat = zvopen(in_unit[i-1],"OPEN_ACT","SA","IO_ACT","SA",0);
        }


    stat = zvget(in_unit[0],"NL",&nl,"NS",&ns,0);	/* Get NL,NS */

    for (i = 1; i < n_inputs; i++)
        {
        stat = zvget (in_unit[i], "NL",&j,"NS",&k,0); /* Check for consistency*/
        if (j != nl || k != ns )
  	    {
...
/* Get DC scale if entered (DC must be entered 5th) */
	...
	getlabval(&in_unit[4],"PICSCALE",&picscale,&stat);
		...
		{
                picscale = 1;
		stat = zvget(in_unit[4],"FORMAT",format,0);
		if (strcmp(format,"BYTE") != 0)
			{
			zvmessage ("Scale of DC frame ambiguous",0);
			zabend();
			}
		}
	}
...
/* Read lines from all inputs (up to 5) and process */
	...
	switch (n_inputs)
	{
	    case 5 :	stat = zvread(in_unit[4], dc, 0);
	    case 4 :	stat = zvread(in_unit[3], rms, 0);
	    case 3 :	stat = zvread(in_unit[2], err, 0);
	    case 2 :	stat = zvread(in_unit[1], sat, 0);
			stat = zvread(in_unit[0], cal, 0);
	}
...
/* Put mean value of slope into slope file label */
    ...
    stat = zvclose(in_unit[0],0);
    stat = zvopen(in_unit[0],"OP","UPDATE","OPEN_ACT","SA","IO_ACT","SA",0);
    stat = zladd(in_unit[0],"HISTORY","ENERGY_UNIT",message,
                "FORMAT","STRING","MODE","REPLACE",0);
	...
	stat = zvclose(in_unit[4],0);
	stat = zvopen(in_unit[4],"OP","UPDATE","OPEN_ACT","SA","IO_ACT","SA",0);
    ...
    zvmessage ("Classifying blemishes...",0);
    i = blemish_total + 1;
...
/*  Summarize  */
    ...
    zvmessage("Histogram of low-full-well pixels",0);
    ...
    stat = zvunit(&out_unit,"OUT",1,0);
    ns = blemish_total * 4;
    stat = zvopen(out_unit,"OP","WRITE","OPEN_ACT","SA","IO_ACT","SA",
	"U_FORMAT","HALF","O_FORMAT","HALF","U_NL",1,"U_NS",ns,0);
    if (bc == 1)
	{
    ...
    stat = zvwrit(out_unit, blemishes, 0);
    stat = zvclose(out_unit, 0);
    zvmessage("BLEMGEN done.",0);
}

init_params()
{
...

{
    ...
    *ind = 0;
    ...
    {
	zvmessage("***Blemish total exceeds allocated storage",0);
...
{ 
    ...
    *ind = zlhinfo(*iunit,tasks,instances,&icnt,"ULEN",TLEN,0);

    for (j=icnt-1; j>=0; j--)
	{
        *ind = zlget(*iunit,"HISTORY",key,value,"HIST",
               tasks[j],"INSTANCE",instances[j],"NELEMENT",1,0);
        if (*ind == 1) return;
	}
