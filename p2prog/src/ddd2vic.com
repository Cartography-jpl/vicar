...
$! VPACK Version 1.9, Sunday, December 31, 2000, 09:11:47
$!
...
main44()
{
  ...
  if ( error == 1 ) return 0;     /* error handling files */

  process_data(nl,ns);          /* read and write ddd data */

  fclose(fp);
  status = zvclose(o_unit,0);
  ...
  int *nl, *ns;	        	/* actual dimensions */
{
  ...
  *ns = s;
  ...
  status = zvopen(o_unit,"OP","WRITE","U_FORMAT","BYTE","O_FORMAT",
	"BYTE","U_NL",*nl,"U_NS",*ns,"OPEN_ACT","SA","IO_ACT","SA",0);
  if ( status != 1 ) goto bad;
  return 0;
...
/* allocate enough room for a line of data */
  ...
  for ( i=0; i<nl; i++ ) /* read and write the data */
      ...
      o_status = zvwrit (o_unit, buf, 0 ); /* and write it out */
      lines = i;
      }

end:
  status = zladd(o_unit,"SYSTEM","NL",&lines,"FORMAT","INT",0);
  return 0;
}
$ VOKAGLEVE
$ Return
