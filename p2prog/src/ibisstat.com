...
$! VPACK Version 1.8, Tuesday, October 28, 1997, 15:36:01
$!
...
C  10-97  SXP   Modified so source lines do not go past column 72 either way
C               that TABs are counted (to prevent compiler errors on HP).
...
4000	CONTINUE
	...
	IF (.NOT. NOPRINT) THEN
	    ...
	    CALL XVMESSAGE (' ',' ')
	    WRITE (STRING,'(A,I6,1X,I6)') 'NUMBER OF DATA POINTS:',NA,NB
	    CALL XVMESSAGE (STRING,' ')
	    CALL XVMESSAGE (' ',' ')
...
7000	CONTINUE
	...
	CALL SEIGEN(MAXCOLS,M,EIGENCORR,EIGENVAL,WORK,EIGENCORR,ERR)
...
C     FAC=DOT(MC,NC,U,J,J)
 ...
 9997 RETURN
      END
      REAL FUNCTION DOT*8(MR,MC,NR,A,JC,KC)
      REAL*8 A(MR,MC)
C
