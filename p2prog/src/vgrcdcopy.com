...
$! VPACK Version 1.8, Friday, April 26, 1996, 13:13:27
$!
...
int main ( int argc, char *argv[])  {
  ...
  long int linesize=836;
  char strbuff[836], in_line[836], out_line[836], label[80][80], sync;
  long int i, j, k, numl1, numl2, vaxhist[256], hist[256], enc_hist[511], total;
  FILE *fp;
#ifdef DEBUG
...
if ( ( longbuf = calloc ( 1, sizeof ( long int))) == NULL)  {
  printf ("Cannot calloc memory for long int buffer\n");
  exit ( 0);
}
...
/*	long ints.  Any conversions are for my own use/debugging	*/
/*									*/
...
for ( i = 0; i < 1; i++)  {
  ...
  status = zvwrit ( outunit, edrptr, "NSAMPS", 1024, 0);
...
/*									*/
status = zvwrit ( outunit, vaxhist, "NSAMPS", 1024, 0);
for ( i = 0; i < 800; i++)  {
  fread ( halfbuf, 2, 1, fp);
  zvtrans ( half_trans, halfbuf, &halftemp, 1);
  fread ( in_line, 1, halftemp, fp);
  if ( (float) halftemp/2 != halftemp/2)  { fread ( &sync, 1, 1, fp); }  /* stay in sync  */
  longtemp = (long int) halftemp;
  ...
  status = zvwrit ( outunit, vline, "NSAMPS", 1024, 0);
}
...
int lsb_trunc;                          /* lsb truncation flag */
  ...
  if (edr->sds.picno[0] != 0) strncpy ( &v02[29], &edr->sds.picno, 10);
  ...
  v03[35] = ascii[filter];
  strncpy ( &v03[37], &fpname[icamera][filter], 6);
  ...
  if (edr->output_volume[0] != 0)  {		/* EDR tape id & file number */
    strncpy ( &v05[3], &edr->output_volume, 6);
...
refgbl $syschar
refgbl $echo
PARM  cd_device TYPE=STRING count=0:1 DEFAULT=--
PARM  cd_volume TYPE=STRING count=0:1 DEFAULT=--
PARM  file_spec TYPE=STRING count=0:1 DEFAULT=--

body
let _ONFAIL="continue"
let $echo="yes"
if ($syschar(1)="UNIX")
   $R2LIB/vgrcdcopy /project/jukebox/voyager/vg_0003/miranda/c2698649.imq +
   test.out
else
  if (not ($count(cd_device) or $count(cd_volume)))
        let $echo="no"
        write "missing parameters.........."
        write "To run this program,"
        write "especify a cd_device, a cd_volume, and a file spec."
        write "e.g.  VICAR>tstvgrcdcopy cd_device = dke100 +"
        write "                         cd_volume = vg_0020+"
        write "                         file_spec = [000000.jupiter.c1940XXX]c1940944.imq"
  else  
        dcl mount/media=cdrom/over=id &cd_device &cd_volume vgrcd_dir
        let $echo="no"
        write " "
        let $echo="yes"
        dcl $vgrcdcp:=="$R2LIB:vgrcdcopy.exe"
        dcl vgrcdcp &cd_device":"&file_spec test.out
        let $echo="no"
        write " "
        let $echo="yes"
        dcl dismount &cd_device
  end-if
end-if
end-proc  
...
.TITLE
    ...
    Test Procedure for the subroutine, vgrcdcopy.
.Help
PURPOSE

    Intended as a unit test for the utility vgrcdcopy.

EXECUTION FORMAT:

     VICAR>tstvgrcdcopy cd_device = dke100    + 
                        cd_volume = vg_0020   +
                        file_spec = [000000.jupiter.c1940XXX]c1940944.imq

OUTPUT FILES
    The output file is test.out.
...
.LEVEL1 
.VARIABLE cd_device 
cd_rom disk drive name.
.VARIABLE cd_volume
cd_volume name.
.VARIABLE file_spec
name and where is the input file
located.  Do not include the 
device name as part of the 
file_spec.

.LEVEL2

.VARIABLE cd_device 
cd_rom disk-drive name. e.g. for CODA2 use DKE100.
.VARIABLE cd_volume
cd_volume name e.g VG_0020
.VARIABLE file_spec
name and where is the input file located.
     e.g [000000.jupiter.c1940XXX]c1940944.imq
End.
