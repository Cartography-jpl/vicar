...
$! VPACK Version 1.7, Wednesday, September 14, 1994, 11:36:32
$!
...
main44()
{
   ...
   zvunit(&out, "OUT", 1, 0);
   zvopen(out, "OP","WRITE","U_NL",nl,"U_NS",ns,0);
...
char string[132];
   ...
   zvget( inp, "NL", &nli, "NS", &nsi, "PIX_SIZE", &pix_size,0);
...
add_label_items()
{
   ...
   status = zladd(out,"HISTORY", "PROJ_LON", &proj_lon, "FORMAT","REAL",0);
   status = zladd(out,"HISTORY", "LON_CL",   &lon_cl,   "FORMAT","REAL",0);
   status = zladd(out,"HISTORY", "LON_CR",   &lon_cr,   "FORMAT","REAL",0);
   status = zladd(out,"HISTORY", "LAT_UC",   &lat_uc,   "FORMAT","REAL",0);
   status = zladd(out,"HISTORY", "LAT_LC",   &lat_lc,   "FORMAT","REAL",0);
   status = zladd(out,"HISTORY", "PROJSAMP", &projsamp, "FORMAT","INT",0);
...
int labelvalue(int unit, char *key, char *type, char *value, int size)
{
   ...
   memset(value,0,size);
   zlhinfo(unit,task,instance,&numtask,0);
   for (j = numtask-1; j >= 0 ; j--) {
      status = zlget (unit,"HISTORY",key,value,"ERR_ACT","","HIST",task[j],
             "INSTANCE",instance[j],"FORMAT",type,"LENGTH",&length,0);
      if (status == 1) break;
...
float    d_lo_po, d_lo_pi, d_ro_lo, d_li_po, d_li_pi, d_ro_pi, d_ri_li;
float    offset_pixels, offset_degrees;

int      offset_pix, buf_size, w_pixels;
int      line, samp, nsamps, status;
int      ss_oi, es_oi, ns_oi, ss_o, es_o, ns_o;
int      ss_i, es_i, ns_i;
...
char     *outbuffer, *to;
   ...
   for (line = sl; line <= el; line++) {
      ...
      if (ns_o == 0) {
         ...
         if (nsamps != 0) {
            zvread(inp, to, "LINE",line,"SAMP",samp,"NSAMPS",nsamps,0);
         }
      }
