...
$! VPACK Version 1.9, Friday, May 01, 1998, 15:51:38
$!
...
c*************************************************************************
	...
	IMPLICIT NONE
        ...
        BYTE E/'E'/                     ! ASCII character E

        BYTE LINE_FEED/Z0A/             ! ASCII line feed




        BYTE      group(80)             ! ascii group ( level ) value

        BYTE      values(80)            ! ascii pair of values
...
c------------------------------------------------------------------------------
	...
	call xvparm( 'FIELDS',  fields,  nfields, idef1, ' ' )
     ...
     +          ')
        ...
        if ( DEB ) then
            call qprint ( '     ' )
            call qprint ( ' ARC2GRAF - Revision C ')
            call qprint ( '     ' )
        end if
...
c------------------------------------------------------------------------------
	...
	if ( status .ne. 1 ) then                       ! Ung   ! Un
   	    call qprint(' Error in xvunit - input')
	    call abend
	endif
	...
	call xvopen(input,status,'COND','NOLABELS','U_FORMAT','BYTE',
     -              'U_NS', 512, 'IO_ACT', 'S ', ' ' )
        if ( status .ne. 1 ) then
            call qprint ( ' Data set open error. Program terminated.')
	    call abend
        end if
...
c                      call mvl (buf(start),group,bytes_per_group)
                  ...
                  decode ( n3, 20, group, iostat=ios, err=200) level
...
c                      call mvl ( inbuf(start),values,bytes_per_pair )
                  ...
                  do l = 1,2
                       decode ( n1, 40, values(ist), 
     -                          iostat=ios, err=200) val
                       if ( l .eq. 1 ) then
                           x = val
                           ist = n1 + 1
                       else
...
c               call mvl ( inbuf(1), buf(start), 512 ) ! put in more data
             ...
             do j = 1, 99999           ! loop through data file
               ...
               do i = index, 512          ! loop through block
                 ...
                 if ( inbuf(i) .eq. E ) then     ! check for END
...
c                       call mvl (buf(start),group,bytes_per_group)
 ...
 20                 format ( I<n3> )
                    ...
                    if ( DEB ) call qprint ( ' Ignored element')
                    go to 50
...
c---------------------------------------------------------------------------
     ...
     -     'LINE', block, 'SAMP', 1, 'NSAMPS', 512, ' ' )
       ...
       if ( status .ne. 1 ) then                       ! Ungen file
   	   call qprint(' Error in xvread. Program terminated.')
	   call abend
       end if
