...
$! VPACK Version 1.8, Tuesday, March 07, 1995, 15:27:16
$!
...
/* Out put ported revision date */
   ...
   zvmessage("BICUBIC version 01-JULY-94", 0 );
...
/* Get the zoom factor */
  ...
  zvunit(&iunit, "INP", first, 0);
  zvunit(&ounit, "OUT", first, 0);

  /* Open the input for array I/O and get size.  */

  zvopen(iunit, "OP", "READ", "OPEN_ACT", "SA", "IO_ACT", "SA", 0 );
  zvsize(&sl, &ss, &nl, &ns, &nli, &nsi,0);
  ...
  if ((nl < 2) || (ns < 2))
  {
    zvmessage("***** There must be at least two lines and samples", 0);
    zvmessage("***** in the image or sub-area.", 0 );
    zabend();
  }
  else
    if (size_changed)   
       zvmessage("***** Sub-area adjusted...", 0 );
  ...
  zvget(iunit, "FORMAT", formatStr, 0 );
