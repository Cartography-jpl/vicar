...
/* loop over images */
 ...
 for(images=1; images <= nids-1; images++){
   ...
   status=zvunit(&unit2,"INP",images+1, NULL);
   ...
   /* open output */
   status=zvunit(&unit3,"OUT",images, NULL);
   status=zvopen(unit3,"OP","WRITE","U_FORMAT","HALF", NULL);

   /* read input into memory */
   for(i=0; i < nl; i++){
     status=zvread(unit2,obuf,"LINE",i+1, NULL);
   ...
   /* write image */
   for(i=0; i < nl; i++){
     status=zvwrit(unit3,&tbuf[i][0],"LINE",i+1, NULL);
   }

   status=zvclose(unit3,"CLOS_ACT","FREE", NULL);
   no_outputs:;
