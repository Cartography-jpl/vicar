...
$! VPACK Version 1.8, Friday, October 24, 1997, 14:48:46
$!
...
/**	status=zvparm("LIGHT",&light,&count,&def,1,0);**/
	...
	status=zvopen(inunit, "OP", "READ", "U_FORMAT", "HALF",0);
        zvsignal(inunit,status,1);
	status=zvopen(outunit, "OP", "WRITE", "O_FORMAT", "HALF", 0);
        zvsignal(inunit,status,1);
	status=zvget(inunit, "NL", &nl, "NS" ,&ns, "RECSIZE", &size,0);
        zvsignal(inunit,status,1);
	...
	imageptr=image;
	for(i=0; i < nl; i++) {
		status=zvread(inunit,imageptr,"LINE",i+1,0);
		zvsignal(inunit,status,1);
		imageptr+=ns;
	}
...
/*******
        ...
        for(i=0, imageptr=image; i < nl; i++)   {
                status=zvwrit(outunit,imageptr,"LINE",i+1,0);
                zvsignal(outunit,status,1);
                imageptr+=nl;
        }
        zabend();
