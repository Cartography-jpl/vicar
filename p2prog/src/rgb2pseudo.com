...
$! VPACK Version 1.8, Monday, March 06, 1995, 09:33:54
$!
...
main44()
{
  ...
  for (i = 1; i <= nInputs; i++)
  {
    zvunit(&inputs[i - 1], "INP", i, 0);

    zvopen(inputs[i - 1], "ADDRESS", &inAddr[i-1],0);
  }
  ...
  zvunit(&output, "OUT", 1, 0);
  zvopen(output, "OP", "WRITE", "U_NL", nl, "U_NS", ns, 
         "U_NB", 1, "ADDRESS", &outFile, 0);
...
/*  putcol(&lutUnit, &1, &256, floatBuf);      */
  ...
  srow   = 1;
  nrows  = 256;
  status = IBISColumnWrite (ibis, floatBuf, column, srow, nrows);
  if (status != 1) {
     IBISSignalU(ibis,status,1);
  }
...
int nl,ns,bits_used;
{
    ...
    bcopy((char *)(table+1),(char *)&temp_table_entry,
	    sizeof(temp_table_entry));
    bcopy((char *)(table+next_free_entry-1),(char *)(table+1),
	    sizeof(temp_table_entry));
    bcopy((char *)&temp_table_entry,(char *)(table+next_free_entry-1),
	    sizeof(temp_table_entry));
    ...
    bcopy((char *)table,(char *)&temp_table_entry,    		/* t <- #0    */
	sizeof(temp_table_entry));
    bcopy((char *)(table+1),(char *)table,	        	/* #0 <- #1   */
	sizeof(temp_table_entry));
    bcopy((char *)(table+255),(char *)(table+1),	        /* #1 <- #255 */
	sizeof(temp_table_entry));
    bcopy((char *)&temp_table_entry,(char *)(table+255),
...
int bits_used,nl,ns,*length_ptr;
{
    ...
    *table_ptr = (struct entry_def *)malloc((unsigned int)*length_ptr *
    ...
    bzero((char *)*table_ptr,*length_ptr * sizeof(struct entry_def));
...
int length,nl,ns,bits_used;
{
	...
	bcopy((char *)(table+source),(char *)(table+next_free_entry),
	    sizeof(*table));
	source++;
    }
	...
	bcopy((char *)(table+length-1),(char *)(table+next_free_entry),
	    sizeof(*table));
	next_free_entry++;
    }
  ...
  int	(*compar)();	/* addr of compare routine	*/
{
  ...
  while (h >= 3)
  {
    ...
    for (i = h; i < nelem; i++)
    {
      bcopy(start + size * i, v, size);
      j = i;
      while ((*compar)(start + size * (j - h), v) > 0)
      {
	bcopy(start + size * (j - h), start + size * j, size);
	j -= h;
	if (j < h) break;
      }
      bcopy(v, start + size * j, size);
    }
  }
  return 1;
}
