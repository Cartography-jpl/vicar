...

{
	...
	// float sgnB=0;
        ...
        // Parameters
        status=zvparm("ITER",&ZMAX,&count,&def,1,0);
        status=zvparm("ALPHA",&alpha,&count,&def,1,0);
	...
	status = zvget(unit,"NS",&ns,"NL",&nl,"NB",&nb,"RECSIZE",&recsize,"ORG",
        ...
        if((ns > 512) || (nl > 512)){
          zvmessage("Input image too large, limits 512 by 512",NULL);
          zabend();
        }
