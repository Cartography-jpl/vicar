...
$! VPACK Version 1.9, Monday, October 07, 2002, 17:59:40
$!
...
c    photb: correct the image using grid (called via stacka) or...
      ...
      call xvmessage('PHOTFUNC version Oct 7, 2002',' ')
      rad=57.29578
...
c
  ...
  115 format('Target center (line,samp)=(',
     &		f12.2,',',f12.2,')')
      call xvmessage(msg,' ')
...
c
     ...
     *' MOSHER PHOTOMETRIC FUNCTION                                     
     *     (0.00-0.00000a+0.00e0p-0.00a)(ci**(0.00+0.0000a)(ce**(00.000.
     *0000a)) '/
      character*70 labvev/
     *' VEVERKA PHOTOMETRIC FUNCTION(00000+0000000A+ 0000E0P-0000A)(CI/C
     *i+ce)'/
      character*62 labmin/
     *' MINNAERT PHOTOMETRIC FUNCTION COSI**000000*(COSE**000000    )'/
      character*84 labbur/
     *' BURATTI PHOTOMETRIC FUNCTION  A=000000 B=000000 C=000000 D=00000
     *0 e=000000 f=000000'/
      character*70 labhap/
     *' HAPKE PHOTOMETRIC FUNCTION,W=000000,B= 000000,H=000000,C= 000000
     *     '/
      character*75 labhap1/
     *' HAPKE PHOTOMETRIC FUNCTION,W=000000,H=000000,TB=000000,S0=000000
     *,hg=000000'/
      character*83 labhap2/
     *' HAPKE PHOTOMETRIC FUNCTION,W=000000,H=000000,TB=000000,S0=000000
     *,b=000000,c=000000'/
      character*70 labcok/
     *' COOK PHOTOMETRIC FUNCTION,W=00000,B= 00000,H=00000,C= 00000,CK= 
     *0    '/
      character *56 labirv/
...
ccc         ck=stars
     ...
     *            'FORMAT','STRING',' ')
      ...
      if(minn.eq.5) then   
	   write(labmos(72:75), '(f4.2)') d(1)		! new
	   write(labmos(77:83), '(f7.4)') -d(2)		! new
	   write(labmos(86:89), '(f4.2)') d(3)		! new
	   write(labmos(93:97), '(f5.2)') d(4)		! new
	   write(labmos(106:109), '(f4.2)') d(5)	! new
	   write(labmos(111:116), '(f6.4)') d(6)	! new
	   write(labmos(125:129), '(f5.2)') d(5)-1	! new
	   write(labmos(131:136), '(f6.4)') d(6)	! new
...
c ********************************************************************
     ...
     +          cmbuf,l4,total,nsox,ncol)
      ...
      character*75 msg1/'   LINE    SAMPLE  LATITUDE LONGITUDE  INC    E
     *MI   PHASE   BRIGHT COLUMN'/
