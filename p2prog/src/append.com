...
$! VPACK Version 1.7, Wednesday, July 20, 1994, 15:08:29
$!
...
main44()
{
  ...
  for(i=0; i<nds; i++) {
    ...
    zvunit(&u[i], "INP", i+1, 0);
    zvopen( u[i], "OPEN_ACT", "SA", "IO_ACT", "SA",0);
    zvget( u[i], "NL", &nli[i], "NS", &nsi, "FORMAT", fcod,0);
  ...
  zvopen( oun, "OP", "WRITE", "U_NL", nline, "U_NS", nsamp, "O_FORMAT", ofmt,
	"U_FORMAT", ofmt, "OPEN_ACT", "SA", "IO_ACT", "SA",0);
  for(i=0; i<nds; i++) {		/* for every file */
    zvopen( u[i], "OPEN_ACT", "SA", "IO_ACT", "SA","U_FORMAT",ofmt,0);
    memset(buf,0,nbyt);
