...
$! VPACK Version 1.8, Wednesday, January 11, 1995, 14:15:10
$!
...
C	 Sets up the column translation stuff
	...
	INTEGER STATUS
	LOGICAL IBIS2
	CHARACTER*10 VERSION,FMT
	CHARACTER*1 CHR
	INTEGER ASIZE

	COUNT = IBIS_FILE_GET( IBIS, 'VERSION', VERSION, 1,1)
	COUNT = IBIS_FILE_GET( IBIS, 'NC', NC, 1,1)
	
	IBIS2 = (VERSION(6:6).EQ.'2')
...
void FTN_NAME(endscreen)()
{
	endwin();
}

void FTN_NAME(emit_beep)()
{
	beep();
}
...
void FTN_NAME(eraseline)()
{
	wclrtoeol(WEdit);
 }


void FTN_NAME(delete_char)()
{
	int y,x;
...
void FTN_NAME(newscreen)()
{
	clear(); 
}


void FTN_NAME(show_help)()
{
	int line=0;
...
void FTN_NAME(show_nokeypad_help)()
{
	int line=0;
...
void FTN_NAME(show_data)()
{
	touchwin( WEdit );
}

void FTN_NAME(clear_data)()
{
   wclear(WData);
   touchwin(WData);
}

void FTN_NAME(show_header)()
{
	touchwin( WEdit );
	wrefresh( WHeader );
}

void FTN_NAME(show_rownum)()
{
	touchwin( WRowNum );
}

void FTN_NAME(show_colnum)()
{
	touchwin( WColNum );
}
...
! Test general ops, and make sure that "junk" is skipped:
! --Added test for correct A8 text handline, FR#85876
!
 edibis a command=(junk,"(2,4) SET 4.567","row 3",+
...
! Test Formatting for old IBIS and searches:
