...
$! VPACK Version 1.8, Tuesday, July 09, 1996, 18:11:36
$!
...
ii = 1;
stay = zvunit(&unit,"INP",ii, 0);    /* Determine unit number of file name */
stay = zvopen(unit,"OPEN_ACT","SA", 0);	/* Open file; if error, abort LABLIST */

print_header_systemlabel(unit,file);	/* Print header and system label      */
/* Get all task names of file         */
stay = zlhinfo(unit,tasks,instances,&numoftasks,"ULEN",MAX_LABEL_KEY_SIZE+1,
	       "ERR_ACT","SA", 0);
...
stay = zvget(u,"FORMAT",frmat,"NL",&nl,"NS",&ns,"NB",&nb,"ORG",org, 0);
return_if_error(u);
...
stay = zlget(u,"HISTORY","LAB01",value,"HIST"   /* Determine label type	     */
	,task, 0);
if(stay != 1) {
	stay = zlget(u,"HISTORY","MISSION",value,"HIST",task, 0);
	if(stay != 1) 
		*type = 2;
...
stay = zlget(u,"HISTORY","NLABS",&numoflabels, "HIST",task, 0);
...
for(n=2;n<=numoflabels;n++)     /* Print all labels  */
	...
	stay = zlget(u,"HISTORY",label,data,"HIST",task, 0);
...
/* Get next value  */
        stay = zlget(unit,"HISTORY",key, info.data,"HIST",&tasks[count][0],
               ...
               "ULEN",info.maxlength,"NELEMENT",info.nelements, 0);
		continue_if_error(unit);
