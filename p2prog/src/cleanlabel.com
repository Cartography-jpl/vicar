...
$! VPACK Version 1.7, Wednesday, May 25, 1994, 11:27:38
$!
...
/* Copy the data */
    while (TRUE) {   
        status = zvread (image_unit, buf, 0) ;
...
******************************************************************************/
{
   (void) zvclose (unit_number, "CLOS_ACT", "FREE", 0) ;
   return ;
}  
...
*/
    do {
        status = zlninfo (invar->inp_unit, one_key, format, &len, &nelem, 0) ;
...
*/
    ...
    status = zlhinfo (invar->inp_unit, invar->task_names, invar->instances,
             &invar->num_tasks, "ULEN", MAX_LABEL_KEY_SIZE+1, 0) ;
    abort_on_error (invar->inp_unit) ;
