...
$! VPACK Version 1.9, Friday, March 02, 2001, 14:45:58
$!
...
status=zvopen(inunit, "");
if (status != 1) my_abort("can not open first input image");

status=zvget(inunit,"nl",&nl,"ns",&ns, "FORMAT", format, "");
...
if (status != 1) 
   ...
   for (lauf_nl=1; lauf_nl <= nl; lauf_nl++)         
      {
      status=zvread(inunit, buf,  "");
      status=zvwrit(outunit, buf, "");
      }
   zvclose(inunit, "");
   }

status=zvclose(outunit,"");
}
