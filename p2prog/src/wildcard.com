...
$! VPACK Version 1.7, Wednesday, September 07, 1994, 10:11:01
$!
...
main44()
{
    unsigned char *name_list[MAX_NAMES] ;
    int count;
...
static void collect_file_names (name_list, count)
  unsigned char *name_list[] ;
  int *count;
{                                    
    int limit, input_cnt, input, def, nbytes, i, status ;
    unsigned char input_string[4096], *current, *newf ;
...
static void find_default_extension (name) 
  unsigned char *name ;        /* process id, i.e. .Zxx where xx are the  */
...
static void search_for_names (name, name_list, count)
  unsigned char *name, *name_list[] ;         /* all files based on name, a */
...
{       
   ...
   unsigned char name_keyword[] = "LOCALVAR";
...
static int get_space (size)
int size;
{                                   /* Allocate the requested space from */
    unsigned char *buffer;          /* virtual memory and zero it out.   */
...
{                                    
    ...
    unsigned char filename[133] ;
    char  mess[80];
    FILE *fileptr;
    unsigned char file_in[4096];
...
*/
    ...
    while ( !feof (fileptr)) {
       ...
       name_list[*count] = (unsigned char *) get_space(length + 1);
