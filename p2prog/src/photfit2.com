...
$! VPACK Version 1.9, Wednesday, March 25, 1998, 14:31:02
$!
...
void main44()
{
  int cnt, def, l, i, j, ival, ival1, num, illMode, status, catReadStatus, count, npts, tnpts=0, ipts=0, n=0, least;
  int row, nrow, allrow=0, ncol, inpCount;
  int class_id, pdfClass_id, classCount;
  int size_FuncVal_array, size_illum_array;
  int lista[phoMAX_PARAM_PER_FUNC];
  int rerun, maxIter, numTen, numNorm, numPrint;
  float temp;
  double dval, dval1, IncAng, EmAng, PhasAng, *phoFuncVal, eps;
  ...
  char *pKeyListMin, *pKeyListMax, *pKeyListT;
  char fileName[phoINP_FILE_NUM][phoFILENAME_LENGTH+1];
/*  void *min, *max, *ptemp;*/
  PHO pho_obj, pho_obj2;
  PHO_ILLUM illum;
...
/*  pkeylist = (char *)malloc( phoMAX_PARAM_PER_FUNC * ( phoMAX_KEYWD_LENGTH+1 ) * sizeof(char));
			...
			zvmessage(" ","");
	    		...
	    		zvmessage("*** memory allocation 1 failed ***","");
	    		free(pkeylist);
	    		zmabend("photfit abend");
		    }
...
/* allocate memory for the parameters, limits,temperaturs */
  ...
  pKeyListMin = (char *)malloc( phoMAX_PARAM_PER_FUNC * ( phoMAX_KEYWD_LENGTH+1+4 ) * sizeof(char));
		    if ( pKeyListMin == NULL) 
		    {
			zvmessage(" ","");
	    		zvmessage("***photfit2 error***","");
	    		zvmessage("*** memory allocation 2 failed ***","");
	    		free(pKeyListMin);
	    		zmabend("photfit abend");
		    }
  pKeyListMin = (char *)keyListMin;

 pKeyListMax = (char *)malloc( phoMAX_PARAM_PER_FUNC * ( phoMAX_KEYWD_LENGTH+1+4 ) * sizeof(char));
		    if ( pKeyListMax == NULL) 
		    {
			zvmessage(" ","");
	    		zvmessage("***photfit2 error***","");
	    		zvmessage("*** memory allocation 3 failed ***","");
	    		free(pKeyListMax);
	    		zmabend("photfit abend");
		    }
  pKeyListMax = (char *)keyListMax;

 pKeyListT = (char *)malloc( phoMAX_PARAM_PER_FUNC * ( phoMAX_KEYWD_LENGTH+1+2 ) * sizeof(char));
		    if ( pKeyListT == NULL) 
		    {
			zvmessage(" ","");
	    		zvmessage("***photfit2 error***","");
	    		zvmessage("*** memory allocation 4 failed ***","");
	    		free(pKeyListT);
	    		zmabend("photfit abend");
		    }
  pKeyListT = (char *)keyListT;
...
/* loop over all phocat files */
  ...
  {
     status = zvunit( VicarUnit+j-1, "inp", j, 0 );
...
/*  strcpy( msg, " illum.cos.inc = " );
  sprintf( cval1, " %10.6e", illum_array[ipts].cos.inc);
  strcat( msg, cval1);
  strcat( msg, " illum.cos.em = " );
  sprintf( cval1, " %10.6e", illum_array[ipts].cos.em);
  strcat( msg, cval1);
  zvmessage( msg, "");
  zvmessage( " ", "");
*/
...
{
  strcpy( msg, " illum.cos.inc = " );
  sprintf( cval1, " %10.6e", illum_array[i].cos.inc);
  strcat( msg, cval1);
  strcat( msg, " illum.cos.em = " );
  sprintf( cval1, " %10.6e", illum_array[i].cos.em);
  strcat( msg, cval1);
  zvmessage( msg, "");
  zvmessage( " ", "");
}
...
/*    for (i=0; i<num; i++) *//* set all parameters to the first parameter guess */
/*    {
     status = phoGetVal( pho_obj2, keylist[i], &dval1);
      status = phoGetVal( pho_obj, keylist[i], &dval1);
      if(status!=phoSUCCESS)
      {
			zvmessage(" ","");
      ...
      status = phoSetVal( pho_obj, keylist[i], dval1);
      if(status<=phoSUCCESS)
      {
			zvmessage(" ","");
	    		zvmessage("***main44 error***","");
	    		zvmessage("*** phoSetVal failed ***","");
sprintf(msg, "status=  %8d"
						, status);
zvmessage(msg,"");
	    		zmabend("photfit abend");
      }
...
*/
    ...
    status = phoMetropolis( pho_obj, limits, phoParT, x, illum_array, tnpts, toleranc,  percent, maxIter, numTen, numNorm, numPrint, &xcost); 
	...
	double *xcost)
{
  ...
  double mincost, costsum, tmp, prob, boltzmann, energy;
