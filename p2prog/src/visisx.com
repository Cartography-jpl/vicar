...
$! VPACK Version 1.9, Saturday, October 29, 2005, 20:03:03
$!
 ...
 */
{
  ...
  vstat = zvopen(inunit,"OP","READ","OPEN_ACT","SA","IO_ACT","SA",0);
...
nonmap:
  ...
  numtasks = 20;
  vstat = zlhinfo(inunit,task,instances,&numtasks,"ERR_ACT","SA",0);
  ...
  if (vcmm_flag) { /* gather some useful parameters for PDS History ... */
    ...
    vstat = zlget(inunit,"HISTORY","SPKERNLS",spkerns,"HIST","VIMSCMM",
     "ULEN", 101, "NELEMENT", -1, "NRET", &nspk, 0);

    vstat = zlget(inunit,"HISTORY","CKERNEL",ckern,"HIST","VIMSCMM",0);
    icker = (vstat>0);

    vstat = zlget(inunit,"HISTORY","DPOINT",dpoint,"HIST","VIMSCMM",
     "NELEMENT", -1, "NRET", &ndpnt, 0);

    nepherr = 0;
    vstat = zlget(inunit,"HISTORY","EPHERRSC",&epherr[0],"HIST","VIMSCMM",0);
    if (vstat>0) nepherr++;
    vstat = zlget(inunit,"HISTORY","EPHERRTG",&epherr[1],"HIST","VIMSCMM",0);
    if (vstat>0) nepherr++;
  }
  ...
  if (!uwaves) {
    vstat = zlget(inunit,"HISTORY","WAVLNTHS",waves,"NRET",&nb1,
     "NELEMENT",-1,"HIST",tasknam,0);
    if (vstat>0 && nb!=nb1) zmabend(" incorrect # of wavelengths!");
  }

  vstat = zlget(inunit,"HISTORY","WAVUNIT",wavunit,"HIST",tasknam,0);
  ...
  vstat = zlget(inunit,"HISTORY","OBSNOTE",v.obsnote,"HIST",tasknam,0);
  fill_if_error(v.obsnote,"String");

  vstat = zlget(inunit,"HISTORY","DATANAME",v.dataname,"HIST",tasknam,0);
  if (vstat<=0) strcpy( v.dataname, "UNKNOWN");

  vstat = zlget(inunit,"HISTORY","DATAUNIT",v.dataunits,"HIST",tasknam,0);
  if (vstat<=0) strcpy( v.dataunits, "UNKNOWN");

  vstat = zlget(inunit,"HISTORY","MULTIPLIER",&core_mult,"HIST",tasknam,0);
  if (vstat<=0) core_mult = 1.0;

  vstat = zlget(inunit,"HISTORY","OBSNAME",v.obsname,"HIST",tasknam,0);
  fill_if_error(v.obsname,"String");
  /* ensure that string is zero-terminated: */
  v.obsname[12] = 0; 

  vstat = zlget(inunit,"HISTORY","PROD_ID",v.prod_id,"HIST",tasknam,0);
  fill_if_error(v.prod_id,"String");

  vstat = zlget(inunit,"HISTORY","PHASE",v.phase_name,"HIST",tasknam,0);
  if (vstat<=0) strcpy( v.phase_name, "UNKNOWN");

  vstat = zlget(inunit,"HISTORY","PROJECT",v.proj_name,"HIST",tasknam,0);
  if (vstat<=0) strcpy( v.proj_name, "UNKNOWN");

  vstat = zlget(inunit,"HISTORY","INSTRMNT",v.ins_name,"HIST",tasknam,0);
  if (vstat<=0) strcpy( v.ins_name, "UNKNOWN");

  vstat = zlget(inunit,"HISTORY","TARGET",v.target,"HIST",tasknam,0);
  if (vstat<=0) strcpy( v.target, "UNKNOWN");

  vstat = zlget(inunit,"HISTORY","PHOT_FNC",v.photofunc,"HIST",tasknam,0);
  ...
  if( v.photofunc[0]=='L' && v.photofunc[1]=='A' ) {
    ...
    vstat = zlget(inunit,"HISTORY","MINN_EXP",&v.minn_exp,"HIST",tasknam,0);
  }

  if (v.radii[0]==v.radii[2] || v.radii[0]<0.0 ) pgraphic = -1;
  else {
    vstat = zlget(inunit,"HISTORY","LATITUDE_TYPE",lattyp,"HIST",tasknam,0);
    if (vstat<=0)
     vstat = zlget(inunit,"HISTORY","LAT_TYPE",lattyp,"HIST",tasknam,0);
    if (!strcmp(lattyp,"PLANETOGRAPHIC")) pgraphic = 1;
    else pgraphic = 0;
  }

  vstat = zlget(inunit,"HISTORY","MAXRANGE",&v.max_range,"HIST",tasknam,0);
  zero_if_error(&v.max_range);

  vstat = zlget(inunit,"HISTORY","MINRANGE",&v.min_range,"HIST",tasknam,0);
  zero_if_error(&v.min_range);

  vstat = zlget(inunit,"HISTORY","MAXLAT",&v.max_lat,"HIST",tasknam,0);
  zero_if_error(&v.max_lat);

  vstat = zlget(inunit,"HISTORY","MAXLON",&v.max_lon,"HIST",tasknam,0);
  zero_if_error(&v.max_lon);

  vstat = zlget(inunit,"HISTORY","MINLAT",&v.min_lat,"HIST",tasknam,0);
  zero_if_error(&v.min_lat);

  vstat = zlget(inunit,"HISTORY","MINLON",&v.min_lon,"HIST",tasknam,0);
  zero_if_error(&v.min_lon);

  vstat = zlget(inunit,"HISTORY","MAXSUN_D",&v.max_sun_d,"HIST",tasknam,0);
  zero_if_error(&v.max_sun_d);

  vstat = zlget(inunit,"HISTORY","MINSUN_D",&v.min_sun_d,"HIST",tasknam,0);
  zero_if_error(&v.min_sun_d);

  vstat = zlget(inunit,"HISTORY","MAX_CB_D",&v.max_cb_d,"HIST",tasknam,0);
  zero_if_error(&v.max_cb_d);

  vstat = zlget(inunit,"HISTORY","MIN_CB_D",&v.min_cb_d,"HIST",tasknam,0);
  ...
  vstat = zlget(inunit,"HISTORY","SUNAZ",&v.azimuth_sun,"HIST",tasknam,0);
  zero_if_error(&v.azimuth_sun);
  if (v.azimuth_sun<0.0) v.azimuth_sun += 360.;  /* PDS standard 0-360 */

  vstat = zlget(inunit,"HISTORY","SCAZ",&v.azimuth_sc,"HIST",tasknam,0);
  zero_if_error(&v.azimuth_sc);
  if (v.azimuth_sc<0.0) v.azimuth_sc += 360.;  /* PDS standard 0-360 */

  vstat = zlget(inunit,"HISTORY","B_SSCLAT",&v.b_ssc_lat,"HIST",tasknam,0);
  zero_if_error(&v.b_ssc_lat);

  vstat = zlget(inunit,"HISTORY","B_SSCLON",&v.b_ssc_lon,"HIST",tasknam,0);
  zero_if_error(&v.b_ssc_lon);
  if (v.b_ssc_lon<0.0) v.b_ssc_lon += 360.;  /* PDS standard 0-360 */

  vstat = zlget(inunit,"HISTORY","E_SSCLAT",&v.e_ssc_lat,"HIST",tasknam,0);
  zero_if_error(&v.e_ssc_lat);

  vstat = zlget(inunit,"HISTORY","E_SSCLON",&v.e_ssc_lon,"HIST",tasknam,0);
  zero_if_error(&v.e_ssc_lon);
  if (v.e_ssc_lon<0.0) v.e_ssc_lon += 360.;  /* PDS standard 0-360 */

  vstat = zlget(inunit,"HISTORY","B_SSLLAT",&v.b_ssl_lat,"HIST",tasknam,0);
  zero_if_error(&v.b_ssl_lat);

  vstat = zlget(inunit,"HISTORY","B_SSLLON",&v.b_ssl_lon,"HIST",tasknam,0);
  zero_if_error(&v.b_ssl_lon);
  if (v.b_ssl_lon<0.0) v.b_ssl_lon += 360.;  /* PDS standard 0-360 */

  vstat = zlget(inunit,"HISTORY","E_SSLLAT",&v.e_ssl_lat,"HIST",tasknam,0);
  zero_if_error(&v.e_ssl_lat);

  vstat = zlget(inunit,"HISTORY","E_SSLLON",&v.e_ssl_lon,"HIST",tasknam,0);
  zero_if_error(&v.e_ssl_lon);
  if (v.e_ssl_lon<0.0) v.e_ssl_lon += 360.;  /* PDS standard 0-360 */

  vstat = zlget(inunit,"HISTORY","INCI_ANG",&v.inci_angle,"HIST",tasknam,0);
  zero_if_error(&v.inci_angle);

  vstat = zlget(inunit,"HISTORY","PHAS_ANG",&v.phas_angle,"HIST",tasknam,0);
  zero_if_error(&v.phas_angle);

  vstat = zlget(inunit,"HISTORY","EMIS_ANG",&v.emis_angle,"HIST",tasknam,0);
  zero_if_error(&v.emis_angle);

  vstat = zlget(inunit,"HISTORY","BEG_SCET",tstr,"HIST",tasknam,0);
  if (vstat<=0) vstat = zlget(inunit,"HISTORY","BEG_UTC",tstr,"HIST",tasknam,0);
  ...
  vstat = zlget(inunit,"HISTORY","END_SCET",tstr,"HIST",tasknam,0);
  if (vstat<=0) vstat = zlget(inunit,"HISTORY","END_UTC",tstr,"HIST",tasknam,0);
  fill_if_error(tstr,"N");      /* event time         */
  if( tstr[0] != 'x' ) {
  ...
  vstat = zlget(inunit,"HISTORY","DN_SDEV",&v.sdband,"HIST",tasknam,0);
  zero_if_error(&v.sdband);

  vstat = zlget(inunit,"HISTORY","GEO_SDEV",&v.sdgeo,"HIST",tasknam,0);
...
closfil:
  ...
  vstat = zvopen(backunit,"OP","READ","OPEN_ACT","SA","IO_ACT","SA",0);
	...
	*nrecs;					/* Number of file records    */
{
  ...
  vstat = zvopen( inunit, "OPEN_ACT", "SA", "IO_ACT", "SA", "OP", "READ",
   "U_NS", BUFFERSIZE, "U_NL", 100, "COND", "NOLABELS",0);

  /* Read first 512 bytes of the PDS label */
  zvread(inunit,buffer,"NSAMPS",BUFFERSIZE,0);
  ...
  vstat = zvopen(inunit,"OPEN_ACT","SA","IO_ACT","SA","OP","READ",
   "U_NS", BUFFERSIZE, "U_NL", *nrecs, "COND","NOLABELS","ADDRESS",&inptr,0);
...
case '\"':    /* Value is a line comment 'Now is the time . . .'   */
  ...
  zladd(unit,"HISTORY",vicaritem,line,"FORMAT","STRING",0);
  break;
...
default:    /* Value is not a line comment or array of values    */
  ...
  case 'D':
    ...
    if (lsef == 0) goto notfound;
    zladd(unit,"HISTORY",vicaritem,realitem,"FORMAT","DOUB",0);
    break;

  case 'R':
    lsef = get_real_value(&realitem[0],bufpoint,&labptr);
    if (lsef == 0) goto notfound;
    zladd(unit,"HISTORY",vicaritem,realitem,"FORMAT","REAL",0);
    break;

  case 'I': 
    lsef = get_integer_value(&intitem[0],bufpoint,&labptr);
    if (lsef == 0) goto notfound;
    zladd(unit,"HISTORY",vicaritem,intitem,"FORMAT","INT",0);
    break;
...
char cubefile[100];			/* Input files to CUBE	     */
{
  ...
  if (iflag) zvunit( &sideunit, "NONE1", 1, "U_NAME", file, 0);
  zvparm( "BACK", file, &iflag, &j, 1, 0);
  if (iflag) zvunit( &backunit, "NONE2", 1, "U_NAME", file, 0);
  zvparm( "BOTT", file, &iflag, &j, 1, 0);
  if (iflag) zvunit( &bottunit, "NONE3", 1, "U_NAME", file, 0);
  ...
  if (dir==TOISIS) {  /* Copy object files to ISIS cube file    */
    ...
    if (!strcmp(forg,"BSQ") || backunit<0) {
      ...
      for (x=0; x<nr0; x++,ln++,inptr+=BUFFERSIZE) {
        ...
        zvwrit(outunit,outbuf,"NSAMPS",BUFFERSIZE,"LINE",ln,0);
      }
      size -= nr0*BUFFERSIZE;
      if (size > 0) {
	...
	if (s3unit<0) {
          ...
          zvwrit(outunit,outbuf,"NSAMPS",size,"LINE",ln,0);
      } }
      if (s3unit>0) {
    ...
    vstat = zvopen(inunit,"OP","READ","OPEN_ACT","SA","IO_ACT","SA",0);
    vstat = zvopen(s2unit,"OP","READ","OPEN_ACT","SA","IO_ACT","SA",0);
    ...
    vstat = zvopen(inunit,"OPEN_ACT","SA","IO_ACT","SA","ADDRESS",&inptr,
      ...
      "U_FORMAT","BYTE","O_FORMAT","BYTE","COND","NOLABELS",0);
    ...
    vstat = zvopen(outunit,"OP","UPDATE","OPEN_ACT","SA","IO_ACT","SA",
       "LAB_ACT","SA","CLOS_ACT","SA","ADDRESS",&outptr,0);

    if (s1unit>0) vstat = zvopen(s1unit,"OP","UPDATE","OPEN_ACT","SA",
     "IO_ACT","SA","LAB_ACT","SA","CLOS_ACT","SA","ADDRESS",&s1ptr,0);

    if (s2unit>0) vstat = zvopen(s2unit,"OP","UPDATE","OPEN_ACT","SA",
     "IO_ACT","SA","LAB_ACT","SA","CLOS_ACT","SA","ADDRESS",&s2ptr,0);
    ...
    zvclose(outunit,0);
    if (s1unit>0) zvclose(s1unit,0);
    if (s2unit>0) zvclose(s2unit,0);

    /* 3rd dimension can be done at the end ... */
    if (s3unit>0) {
      vstat = zvopen(s3unit,"OP","UPDATE","OPEN_ACT","SA","IO_ACT","SA",
       "LAB_ACT","SA","CLOS_ACT","SA","ADDRESS",&s3ptr,0);
...
post_map:
  ...
  if (lsef) {
    ...
    zladd( unit, "HISTORY", "B_SSCLON", &ritem, "FORMAT", "REAL",0);
  }
  lptr = lptrq;
  ...
  if (lsef) {
    get_real_value( &ritem, bufpoint, &lptr);
    if (conv_lon) ritem = 360.-ritem;
    zladd( unit, "HISTORY", "E_SSCLON", &ritem, "FORMAT", "REAL",0);
  }
  lptr = lptrq;
  ...
  if (lsef) {
    get_real_value( &ritem, bufpoint, &lptr);
    if (conv_lon) ritem = 360.-ritem;
    zladd( unit, "HISTORY", "B_SSLLON", &ritem, "FORMAT", "REAL",0);
  }
  lptr = lptrq;
  ...
  if (lsef) {
    get_real_value( &ritem, bufpoint, &lptr);
    if (conv_lon) ritem = 360.-ritem;
    zladd( unit, "HISTORY", "E_SSLLON", &ritem, "FORMAT", "REAL",0);
  }
  return 1;
}
$ VOKAGLEVE
