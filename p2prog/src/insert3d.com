...
$! VPACK Version 1.9, Friday, February 28, 2003, 14:52:41
$!
...
close_the_files ( in1, in2, out, update )
   ...
   FLAG update ;
{
   ...
   zvclose ( in1 , 0) ;
   zvclose ( in2 , 0) ;
   if ( !update) zvclose ( out, 0 ) ;

   return ;
}


copy_bands ( in, out, sb, eb, tob, nl, ns )

   int in, out, sb, eb, tob, nl, ns ;
{
   ...
   for ( band = sb; band <= eb; ++band, ++tob )
      ...
      {
         zvread ( in, in_line, "BAND", band, "LINE", line , 0) ;
         zvwrit ( out,in_line, "BAND", tob, "LINE", line , 0) ;
      }
...
insert_the_band ( inunit1, inunit2, outunit, band, 
   ...
   int band, nl, ns, nb, inunit1, inunit2, outunit ;
   FLAG overwrite, update ;
{
...
open_the_files ( in1, in2, out, nl, ns, nb, overwrite, update ) 

   int *in1, *in2, *out, *nl, *ns, *nb ;
   FLAG update, overwrite ;
{
   ...
   if ( update ) {
      ...
      zvopen ( *in1, "OPEN_ACT", "SA", "U_ORG", "BSQ", "IO_ACT", "SA" , 0) ;
   }

   zvopen ( *in2, "OPEN_ACT", "SA", "U_ORG", "BSQ", "IO_ACT", "SA" , 0) ;

   zvget ( *in1, "NL", nl, "NS", ns, "NB", nb , 0) ;   
   zvget ( *in2, "NL", &nl2, "NS", &ns2, "NB", &nb2 , 0) ;   
   ...
   if ( !update ) {
       zvopen ( *out, "OP", "WRITE", "U_ORG", "BSQ",
                "OPEN_ACT", "SA", "IO_ACT", "SA", "U_NB", nbands, 0);
   }
