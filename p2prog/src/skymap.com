$!****************************************************************************
$!
$! Build proc for MIPL module skymap
$! VPACK Version 1.9, Monday, December 22, 1997, 14:07:47
$!
$! Execute by entering:		$ @skymap
$!
...
$!****************************************************************************
$!
$ write sys$output "*** module skymap ***"
$!
$ Create_Source = ""
$ Create_Repack =""
$ Create_PDF = ""
...
$ write sys$output "Invalid argument given to skymap.com file -- ", primary
...
$!
$Run_Make_File:
$   if F$SEARCH("skymap.imake") .nes. ""
$   then
$      vimake skymap
$      purge skymap.bld
$   else
$      if F$SEARCH("skymap.bld") .eqs. ""
$      then
$         gosub Imake_File
$         vimake skymap
$      else
...
$      @skymap.bld "STD"
$   else
$      @skymap.bld "''primary'" "''secondary'"
$   endif
$ Return
$!#############################################################################
$Repack_File:
$ create skymap.repack
$ DECK/DOLLARS="$ VOKAGLEVE"
$ vpack skymap.com -
	-s skymap.c header.h qfile.h newgetxy.c qread.c xypos.c readhdr.c -
...
/* write zero's in the output */
   ...
   for(j=1; j <= nl; j++){
     status=zvwrit(ounit,obuf,"LINE",j,0);
     zvsignal(ounit,status,1);
   }

/* reopen output for update */
   status=zvclose(ounit,0);
   zvsignal(ounit,status,1);
...
/********************* MAIN PICTURE LOOP ***************************/
 for(file = 1; file < 999999; file++){
   ...
   status=zvunit(&unit,"OLD",file,"U_NAME",pix_filename,0);
   zvsignal(unit,status,1);
   status=zvopen(unit,"U_FORMAT","HALF",0);
   zvsignal(unit,status,1);
   status=zvget(unit,"NL",&nl_in,"NS",&ns_in,0);
   zvsignal(unit,status,1);
...
/* read input picture into memory */
   for(j=0; j<nl_in; j++){
     status=zvread(unit,&image[j][0],"LINE",j+1,0);
     zvsignal(unit,status,1);
   }
...
/* map project image */
   for(j=1; j <= nl; j++){
     ...
     dline=j;
     status=zvread(ounit,&obuf[1],"LINE",j,0); /* re read output line */
     ...
     status=zvwrit(ounit,&obuf[1],"LINE",j,0);
   }
   status=zvclose(unit,"CLOS_ACT","FREE",0); /* input picture */
...
$Imake_File:
$ create skymap.imake
#define  PROGRAM   skymap

#define MODULE_LIST skymap.c newgetxy.c qread.c xypos.c readhdr.c utility.c \
...
!
! TESTERS:
! create two files "picture.list" and "catalogue.list" containing
! the file names created by sky and skycat.
!
! Picture.list looks like:
! picture1.img
! picture2.img
! picture3.img
! picture4.img
...
!
! Then run this:
sky out=picture1.img ra=42 dec=-30 fov=61. grid=0.0
skycat inp=picture1.img out=catalogue1.ascii
sky out=picture2.img ra=41 dec=-30 fov=61. grid=0.0
skycat inp=picture2.img out=catalogue2.ascii
sky out=picture3.img ra=43 dec=-30 fov=61. grid=0.0
skycat inp=picture3.img out=catalogue3.ascii
sky out=picture4.img ra=42 dec=-29 fov=61. grid=0.0
skycat inp=picture4.img out=catalogue4.ascii
sky out=picture5.img ra=42 dec=-31 fov=61. grid=0.0
skycat inp=picture5.img out=catalogue5.ascii
gen out=b.img nl=10 ns=10
! 57.295 = 360/2pi so 1 km =1 degree
! .0005  = degrees or km / pixel (for the input image)
MAP3 b.img ster_target.img NL=2000 NS=2000 'STEREOGR SCAL=.002 +
   LINE=1000 SAMP=1000 LATI=-30. LONG=42. +
  nora=0 'NOGEOM 'NOSEDR 'NOPROJEC radii=(57.295,57.295,57.295) +
 ommatrix=(1.,0.,0.,0.,1.,0.,0.,0.,1.) rsvector=(1.,1.,1.) +
 cscale=1 focl=1
! create map 
skymap inp=(ster_target.img,picture.list,catalogue.list) +
