...
$! VPACK Version 1.8, Wednesday, May 14, 1997, 14:13:27
$!
...
c initialize & load naif spice kernels
      if(xvptst('SPICE'))then
         call clpool
         call xgetenv_vic('LEAPSECONDS',msg)
         call ldpool(msg)
         if(failed()) call xvmessage
     +   ('***Error loading LEAPSECONDS kernel',' ')
         call xgetenv_vic('CONSTANTS',msg)
         call ldpool(msg)
         if(failed()) call xvmessage
...
c*******************************************************************
     ...
     + form='FORMATTED',iostat=ios,status='UNKNOWN')
      ...
      write(unit=10,fmt=*,iostat=ios)
     +'  ID linel  sampl  liner  sampr   dnl    dnr      X           Y
     +         Z        lat        lon        R        dR        error'
      if(ios.gt.0)then
          call xvmessage('write error on headers',' ')
          call abend
      endif
