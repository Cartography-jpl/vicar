...
$! VPACK Version 1.9, Friday, May 01, 1998, 15:49:38
$!
...
c------------------------------------------------------------------------------
	...
	call xvparm( 'FIELDS',  fields,  nfields, idef1 )   ! input format
...
c------------------------------------------------------------------------------
 ...
 10         continue                   ! get coordinate pair
  ...
  110       format ( 'END' ) 
            close ( unit=output(j), DISP='KEEP' )! close output file
...
c---------------------------------------------------------------------------
       ...
       if ( POINT ) then
           ...
           write ( output(j), 10 ) group_no, data(1), data(2)
 10        format ( I<fields(1)>,2F<fields(2)>.<fields(3)>)

       else

           write ( output(j), 20 ) group_no 
 20        format ( I<fields(1)>)

           do i = 1, number_of_pairs
               write ( output(j), 30 ) data(ist), data(ist + 1)
 30            format ( 2F<fields(2)>.<fields(3)>)
               ist = ist + 2
           end do     
