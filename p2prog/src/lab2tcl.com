...
$! VPACK Version 1.7, Friday, August 05, 1994, 08:33:43
$!
...
!
	...
	integer MAX_LABEL_ITEM_SIZE, MAX_REQ, MAX_NTASKS, FNDKEY
	parameter (
	...
	logical xvptst		! routine to test for parameter switch
	integer unit,		! i/o unit for input file
	...
	integer int_value	! generic label value: integer
	real real_value		! generic label value: real
...
! to last entry.
!
	integer num_tasks,	! number of tasks found in image label
	...
	character tasks(MAX_NTASKS)*8 ! task names
        ...
        call ifmessage('LAB2TCL version 31 OCT 1994')
...
! massive read. We should get the second versions of the repeated keywords.
	...
	lab2tcl image2   +
	    ...
	    (LBLSIZE,FORMAT,TYPE,BUFSIZ,DIM,EOL,RECSIZE,ORG,  +
	    nl,ns,nb,nbb,nlb,R,Yummy,i,newPromp,c,SInc,LInc) 'ALL
