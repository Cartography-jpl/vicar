...
$! VPACK Version 1.9, Wednesday, June 16, 2004, 13:35:47
$!
...
C
      ...
      CALL XVMESSAGE('NAV version Oct 16, 2002',' ')
...
c IF NON THERE, PROMPT USER
      ...
      if (sclon.lt.-999.) then
        ...
        if (ctest.eq.0) then
          call xvmessage('Enter sub-s/c lon (centric,deg)',' ')
          read(*,'(f9.3)') sclon
        endif
      endif
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC72
     ...
     &    ('Enter sub-solar latitude (deg), -999. for s/c lat',' ')
      ...
      if (sunlon.lt.-999.) then
        ...
        if (ctest.eq.0) then
          call xvmessage
     &    ('Enter sub-solar longitude (cent,deg), -999. for s/c lon',
     &    ' ')
          read(*,'(f9.3)') sunlon
        endif
      endif
...
116         FORMAT('Reference plane is that of ',A1,'-Ring')
     ...
     &               NINT(NSDS/ZOOM),0,0,0,0,mindis,maxdis,RPTS,NRPTS) 
               IF (NRPTS.EQ.0) THEN
                  CALL XVMESSAGE('***Ring is not in picture',' ')
               ELSE
...
CBTC cursor is centerpoint of scan, calculate phase and plot points
      ...
      IF ( XVIPTST('CENTERPT') 
     &  .or. ( XVIPTST('REFRESH') .and. (igotem.and.1) .eq. 1)
     &  ) THEN
...
CBTC corner point of scan wrt center point determines length & width of scan
      ...
      IF ( XVIPTST('CORNERPT') 
     &  .or. ( XVIPTST('REFRESH') .and. (igotem.and.2) .eq. 2)
     &  ) THEN

        if ( (igotem.and.1) .ne. 1) then
   ...
   20 CALL XVINTRACT('STARFIT','STARFIT ')
     ...
     &		 CALL DRAWSTARS(STARS,SPTS,NSTARS,LOMAG,HIMAG,1)
      ...
      IF (XVIPTST('TIEPOINT')) THEN
          CALL TIESTAR(*20)
          CALL GETSTARS(ISAO,CM,OAL,OAS,ZSCALE,FOV,T,
...
WRITE "==================================================="
WRITE "FIRST ALLOCATE A DISPLAY DEVICE VIA THE USE COMMAND"
WRITE "EXAMPLE : USE XWC0"
WRITE "==================================================="
WRITE ""
...
! Test SAO star catalog w/Cassini image
NAV w1364364259.2 SPICEMODE=REMOTE TARGET=SATURN SAO=/project/vgr/sao_idl.str
'star
stretch=(15,40)
'exit
'exit
