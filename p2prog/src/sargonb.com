...
$! VPACK Version 1.9, Sunday, August 11, 2002, 12:46:46
$!
...
C					!DEFAULTS:
     ...
     &	      INDEX(FBUF,'divide').ne.0) THEN 
      ...
      ELSE IF (COUNT .LT. 6) THEN
               CALL XVMESSAGE('NEEDS AT LEAST 3 PAIRS OF VERTICES',' ')
               CALL ABEND
      END IF      
      NVERT(NAREA)=COUNT/2
      END DO
...
C
      RETURN
C  998 RETURN 1
970   CALL XVMESSAGE ('*** Invalide CONST value.',' ')
999   CALL XVMESSAGE ('***SARGONB task cancelled',' ')
      CALL ABEND
      END
C
C***************************************************************
