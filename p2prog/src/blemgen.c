...
void flag_blemishes(int *ind, float cal[], short sat[], short err[],
		    short rms[], short dc[], int ns, int line);
...
/* Get DC scale if entered (DC must be entered 5th) */
	...
	getlabval(&in_unit[4],"PICSCALE",&picscale,&stat);
		...
		if (strcmp(format,"BYTE") != 0)
			{
			zvmessage ("Scale of DC frame ambiguous",NULL);
			zabend();
			}
		}
	}
...
/* Put mean value of slope into slope file label */
    ...
    zvmessage ("Classifying blemishes...",NULL);
    i = blemish_total + 1;
...
/*  Summarize  */
    ...
    zvmessage("Histogram of low-full-well pixels",NULL);
...

void flag_blemishes(ind, cal, sat, err, rms, dc, ns, line)
...

void store_blemish(ind,line,samp,criteria,sat_val)
...

{
    ...
    *ind = 0;
    ...
    {
	zvmessage("***Blemish total exceeds allocated storage",NULL);
...

void classify_blemish(index,nl,ns)
...

int is_double_column(line,samp,ns,index)
...

int is_blemish(line,samp)	/* returns true if point is in blemishes array	*/
 ...
 */
void getlabval(iunit,key,value,ind)
    int *iunit,*value,*ind;
    char key[];
{ 
    int icnt, j;
    ...
    *ind = zlhinfo(*iunit,tasks,instances,&icnt,"ULEN",TLEN,NULL);
