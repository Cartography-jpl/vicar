...
$! VPACK Version 1.8, Friday, October 30, 1998, 20:06:32
$!
	...
	DO ICOL = 1, NCOL
	  IF (ICOL .NE. INDEXCOL) THEN
	     ...
	     IF (CFORM(1:1).EQ.'A') THEN !dont format ASCII
	       CALL IBIS_COLUMN_SET(IBISIN,'U_FORMAT','NONE',ICOL,STATUS)
    	       IF (STATUS.NE.1) CALL IBIS_SIGNAL(IBISIN,STATUS,1)
	       CALL IBIS_COLUMN_SET(IBISOUT,'U_FORMAT','NONE',ICOL,STATUS)
	       IF (STATUS.NE.1) CALL IBIS_SIGNAL(IBISOUT,STATUS,1)
	     ENDIF
