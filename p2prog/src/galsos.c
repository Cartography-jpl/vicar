...
errmsg = 0;
zvparm("EXPOSURE",&t,&icnt,&idef,0,NULL);
...
static void getlabval(int iunit,char *key,int *value,int *ind)
{
  ...
  *ind=zlhinfo(iunit,tasks,instances,&icnt, NULL);
...
int *mission_phase;	/* 1=ground calibration, 2=phase1, 3=phase2 */
{
  ...
  *badlabel = *blemcor = 0;
  zlhinfo(*iunit,taskname,instant,&ntask, NULL);
  for (i=0; i<ntask; i++) {
  ...
  able_86_typ *gll;
{
  ...
  if (nlb > 0) ind=zvopen(*ounit,"OP","WRITE","U_FORMAT","HALF","O_FORMAT",
	...
	/* Add label items */
  nhist = 1;
  ind = zlhinfo(*ounit,first_task,&instances,&nhist, NULL);
