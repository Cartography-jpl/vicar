...
$! VPACK Version 1.9, Friday, October 06, 2000, 15:18:00
$!
...
/* Open input file */
zvunit(&unit, "inp", 1, 0);
status = zvopen(unit, "open_act", " ", "io_act", "SA", 0);
...
if (status != SUCCESS) {		/* Open unlabeled file */
	...
	zvadd(unit, "U_FORMAT", form, "I_FORMAT", form, 0);
    }

    zvparm("insize", nl, &count, &def, 3, 0);
    if (!def)
	zvadd(unit, "u_nl", nl[0], "u_ns", nl[1], "u_nb", nl[2], 0);
    ...
    if (!def)
	zvadd(unit, "org", orgin, 0);

    zvopen(unit, "open_act", "sa", "io_act", "sa", "cond", "nolabels", 0);
}

zvget(unit, "nl", &nli, "ns", &nsi, "nb", &nbi, 0);
zvget(unit, "format", form, "org", orgin, 0);
...
nlab = MTASKS;
status = zlhinfo(unit, tasks, inst, &nlab, "ulen", 10, "err_act", "", 0);
if (status != SUCCESS) {
...
if (nlab > 0) {
    zlget(unit, "HISTORY", "DAT_TIM", date1, "err_act","s",
		"hist", tasks[0], "instance", inst[0], "ulen", 33, 0);
    zlget(unit, "HISTORY", "USER", user1, "err_act","s",
		"hist", tasks[0], "instance", inst[0], "ulen", 10, 0);
}
if (nlab > 1) {
    zlget(unit, "HISTORY", "DAT_TIM", daten, "err_act","s",
		"hist", tasks[nlab-1], "instance", inst[nlab-1], "ulen", 33, 0);
    zlget(unit, "HISTORY", "USER", usern, "err_act","s",
		"hist", tasks[nlab-1], "instance", inst[nlab-1], "ulen", 10, 0);
}
	...
	for (slice2 = sl2st; slice2 <= l2; slice2 += sl2inc) {
		...
		for (s1 = sl1st; s1 <= sl1end; s1 += sl1inc) {
		    zvread(unit, buf+i, sl2_cmd, slice2,
			   sl3_cmd, slice3, sl1_cmd, s1, n_cmd, 1, 0);
		    i += psin;		/* increment by pixel size */
		}
	    }
	    else {
		zvread(unit, buf, sl2_cmd, slice2, sl3_cmd, slice3, 0);
