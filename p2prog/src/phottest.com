...
$! VPACK Version 1.8, Friday, March 15, 1996, 09:51:23
$!
...
/* GENERAL_QLF */   for(i=0;i<n_gqlf;i++)
			...
			strcpy(format_buf[i+n_opc+n_imgrps*(2+n_ipqlf)],
                      (gqlf_fo+i*IFMT_SIZE));
                       ...
                       /*************************/
/* VICAR unit */    zvunit(&vicar_unit,"none",1,"U_NAME",filename,0);
...
/* Write the data into the file (in a bad style) */
  ...
  ftemp[0] = IncAng;
  status = IBISColumnWrite( IbisUnit, ftemp, col, row, 1);
	if (status <= 0) IBISSignal (IbisUnit, status, 1 );
  ...
  count = IBISColumnFind( IbisUnit, ITYPE_GROUP, ctmp1, &col, 1, 1);
  	if (count != 1) IBISSignal( IbisUnit, count, 1 );
  itemp[0] = class_id;
  status = IBISColumnWrite( IbisUnit, itemp, col, row, 1);
	if (status <= 0) IBISSignal (IbisUnit, status, 1 );
...
/* get the photometric function name : */
  ...
  "property=PHOT keyword 'PHO_FUNC' was not added",0);
...
/* get the parameters of the current photometric function : */
  ...
  for ( j=0; j<num; j++ )  /* for all function parameters */
  {
    ...
    "ERR_MESS","property=PHOT parameter was not added", 0 );
...
/* get the name of the photometric function */
	...
	"keyword 'PHO_FUNC' was not read",0);
	e[0]=status;
 	...
 	for (j=0; j<num; j++) /* for all function parameters */
  	{
	   ...
	   "ERR_MESS","property=PHOT parameter  was not read",0);
	   e[j+1]=status;
