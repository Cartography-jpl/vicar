...
$! VPACK Version 1.9, Friday, October 10, 2003, 12:47:29
$!
...
main44()
{
	...
	/* Open output file */
	status = zvunit( &unit_number,"OUT",1,0 );
	ABENDif( status<picSUCCESS );
	
	status = zvopen( unit_number,"OP","WRITE",
			"OPEN_ACT","SA",
   			"U_NL",NL,"U_NS",NS,"U_NB",NB,0);
	ABENDif( status<picSUCCESS );
     ...
     else if (NB == 3)
     {
       ...
       for (j = SL-1; j < NL; ++j)
       {
	    ...
	    zvwrit(unit_number, &blu[lineStart],"LINE",j+1,
                "SAMP",1,"NSAMPS",NS,"BAND",3,0);
       }
     }

	status = zvclose( unit_number, 0 );		/* Close input file */
	ABENDif(status<picSUCCESS);
	free(red);
        return;
}
...
int	status;
 int  tempBand1, tempBand2;
	...
	CHECKif(status<picSUCCESS);
     ...
     status = zvsize (&user_parameters->sl,&user_parameters->ss,
